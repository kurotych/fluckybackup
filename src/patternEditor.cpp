...
patternEditor::patternEditor(QString sourceRoot, QWidget *parent)
    : QDialog(parent) {
  uiP.setupUi(this);
  ...
  if (!SourceRoot.endsWith(SLASH)) {
    anchorSource =
        sourceRoot.right(sourceRoot.size() - sourceRoot.lastIndexOf(SLASH));
  ...
  connect(uiP.toolButton_browseFile, SIGNAL(clicked()), signalMapper,
          SLOT(map()));
  signalMapper->setMapping(uiP.toolButton_browseDir, 1);
  connect(uiP.toolButton_browseDir, SIGNAL(clicked()), signalMapper,
          SLOT(map()));
  ...
  connect(uiP.radioButton_fileSpecific, SIGNAL(clicked(bool)), this,
          SLOT(fillPattern()));
  connect(uiP.radioButton_directorySpecific, SIGNAL(clicked(bool)), this,
          SLOT(fillPattern()));
  connect(uiP.radioButton_fileSet, SIGNAL(clicked(bool)), this,
          SLOT(fillPattern()));
  connect(uiP.radioButton_directorySet, SIGNAL(clicked(bool)), this,
          SLOT(fillPattern()));
  connect(uiP.lineEdit_browse, SIGNAL(textChanged(QString)), this,
          SLOT(fillPattern()));
  connect(uiP.spinBox_pathDeeper, SIGNAL(valueChanged(int)), this,
          SLOT(fillPattern()));
  connect(uiP.checkBox_directoryContents, SIGNAL(clicked(bool)), this,
          SLOT(fillPattern()));
  connect(uiP.lineEdit_nameIsExactly, SIGNAL(textChanged(QString)), this,
          SLOT(fillPattern()));
  connect(uiP.lineEdit_nameStartsWith, SIGNAL(textChanged(QString)), this,
          SLOT(fillPattern()));
  connect(uiP.lineEdit_nameEndsWith, SIGNAL(textChanged(QString)), this,
          SLOT(fillPattern()));
  connect(uiP.lineEdit_nameContains, SIGNAL(textChanged(QString)), this,
          SLOT(fillPattern()));
  connect(uiP.radioButton_fileSpecific, SIGNAL(toggled(bool)), this,
          SLOT(fileRadioChanged()));
  connect(uiP.checkBox_pathAny, SIGNAL(clicked(bool)), this,
          SLOT(fillPattern()));
  connect(uiP.checkBox_pathAbsolute, SIGNAL(clicked(bool)), this,
          SLOT(fillPattern()));
}
// SLOTS-------------------------------------------------------------------------------------
// --------------------------------okay
...
// --------------------------------cancel
// pressed------------------------------------------------
void patternEditor::cancel() { close(); }

void patternEditor::closeEvent(QCloseEvent *event) { event->accept(); }
...
void patternEditor::browse(const int type) {
  ...
  case 0:
    newLineEdit =
        QFileDialog::getOpenFileName(this, tr("Select file"), SourceRoot);

    if (newLineEdit !=
        "") // if something is selected indeed (not cancel pressed)
    {
      if (notXnixRunning)
        newLineEdit.replace("/",
                            SLASH); // The OS2 filedialog captures path with
                                    // *nix slashes. Change it to OS2 style

      if (!newLineEdit.startsWith(SourceRoot)) {
        textDialog textdialogW("QtWarning",
                               tr("Please choose a file within the source") +
                                   " :<br><br>" + SourceRoot,
                               this);
        textdialogW.exec();
      } else
  ...
  case 1:
    newLineEdit = QFileDialog::getExistingDirectory(
        this, tr("Select directory"), SourceRoot);
        ...
        "") // if something is selected indeed (not cancel pressed)
    {
      ...
      if (!newLineEdit.startsWith(SourceRoot)) {
        textDialog textdialogW(
            "QtWarning",
            tr("Please choose a directory within the source") + " :<br><br>" +
                SourceRoot,
            this);
        textdialogW.exec();
      } else
...
void patternEditor::fillPattern() {
  ...
  if ((uiP.radioButton_fileSpecific->isChecked()) ||
      (uiP.radioButton_directorySpecific->isChecked())) {
    uiP.lineEdit_browse->setEnabled(true);
  ...
  if (uiP.lineEdit_browse->isEnabled()) {
    patternLine = uiP.lineEdit_browse
                      ->text(); // first get the string from the path lineEdit
    if (patternLine.startsWith(SourceRoot))
      patternLine.replace(
          SourceRoot,
          anchorSource +
              SLASH); // replace the starting source in the pattern with /
  }

  // go deeper in path -------------------
  // -------------------------------------------------------------------------------
  if ((uiP.checkBox_pathAny->isEnabled()) &&
      (uiP.checkBox_pathAny->isChecked()))
    patternLine.append("**" + SLASH);
  else {
    count = 0;
    while ((count < uiP.spinBox_pathDeeper->value()) &&
           (uiP.spinBox_pathDeeper->isEnabled())) {
      patternLine.append("*" + SLASH);
      count++;
    }
  }
  ...
  if ((uiP.lineEdit_nameStartsWith->isEnabled()) &&
      (!(uiP.lineEdit_nameStartsWith->text() == "")))
    patternLine.append(uiP.lineEdit_nameStartsWith->text() + "*");

  if ((uiP.lineEdit_nameContains->isEnabled()) &&
      (!(uiP.lineEdit_nameContains->text() == ""))) {
    if (uiP.lineEdit_nameStartsWith->text() == "")
      patternLine.append("*");
    patternLine.append(uiP.lineEdit_nameContains->text() + "*");
  }

  if ((uiP.lineEdit_nameEndsWith->isEnabled()) &&
      (!(uiP.lineEdit_nameEndsWith->text() == ""))) {
    if ((uiP.lineEdit_nameContains->text() == "") &&
        (uiP.lineEdit_nameStartsWith->text() == ""))
      patternLine.append("*");
  ...
  // ----------------------------------------------------------------------------------------------------
  if (uiP.radioButton_fileSpecific
          ->isChecked()) // if file specific radio button is selected
  {
  ...
  if (uiP.radioButton_directorySpecific
          ->isChecked()) // if directory specific radio button is selected
  {
    uiP.label_browse->setVisible(true);
    uiP.label_browse->setText(tr("directory :"));
  ...
  if (uiP.radioButton_fileSet
          ->isChecked()) // if file set radio button is selected
  {
  ...
  if (uiP.radioButton_directorySet
          ->isChecked()) // if directory set radio button is selected
  {
    uiP.checkBox_pathAbsolute->setEnabled(true);
    uiP.label_browse->setVisible(false);
    if ((!(patternLine.endsWith(SLASH))) && (!(patternLine == "")))
  ...
  // -------------------------------------------------------------------------------
  if ((uiP.checkBox_directoryContents->isChecked()) &&
      (uiP.checkBox_directoryContents->isEnabled()))
    patternLine.append("**");

  if (notXnixRunning)
    patternLine.replace(SLASH, "/"); // replace OS2 slashes with *nix ones ->
                                     // OS2 uses *nix slashes for patterns

  uiP.lineEdit_patternFinal->setText(patternLine);
}
// fileRadioChanged----------------------------------------------------------------------------
// actions when the file specific radio button is toggled
void patternEditor::fileRadioChanged() { uiP.lineEdit_browse->setText(""); }

// --------------------------------accessors-----------------------------------
QString patternEditor::getPattern() { return itsPattern; }
