/* ---------------------------------- winDialog.cpp
-------------------------------------------------------------------------- Class
to display a dialog and declare rsync and ssh paths
...
// class winDialog
// Constructor=================================================================================================
// Displays a simple dialog with text info, ok-cancel buttons and lineEdits for
// user input
winDialog::winDialog(QWidget *parent) : QDialog(parent) {
  ...
  connect(uiD.toolButton_defaultRsync, SIGNAL(clicked()), signalMapper2,
          SLOT(map()));
  signalMapper2->setMapping(uiD.toolButton_defaultSsh, 1);
  connect(uiD.toolButton_defaultSsh, SIGNAL(clicked()), signalMapper2,
          SLOT(map()));
  signalMapper2->setMapping(uiD.toolButton_defaultCygpath, 2);
  connect(uiD.toolButton_defaultCygpath, SIGNAL(clicked()), signalMapper2,
          SLOT(map()));
  signalMapper2->setMapping(uiD.toolButton_defaultDosdev, 3);
  connect(uiD.toolButton_defaultDosdev, SIGNAL(clicked()), signalMapper2,
          SLOT(map()));
  signalMapper2->setMapping(uiD.toolButton_defaultVshadow, 4);
  connect(uiD.toolButton_defaultVshadow, SIGNAL(clicked()), signalMapper2,
          SLOT(map()));
  signalMapper2->setMapping(uiD.toolButton_defaultTempdir, 5);
  connect(uiD.toolButton_defaultTempdir, SIGNAL(clicked()), signalMapper2,
          SLOT(map()));
  signalMapper2->setMapping(uiD.toolButton_defaultAppdir, 6);
  connect(uiD.toolButton_defaultAppdir, SIGNAL(clicked()), signalMapper2,
          SLOT(map()));
  connect(signalMapper2, SIGNAL(mapped(const int)), this,
          SLOT(useDefault(const int)));
...
// --------------------------------cancel
// pressed------------------------------------------------
void winDialog::cancel() { close(); }

void winDialog::closeEvent(QCloseEvent *event) { event->accept(); }
...
void winDialog::browse(const int type) {
  ...
  case 0:
    newLineEdit = QFileDialog::getOpenFileName(
        this, tr("Select rsync command", "file selection dialog title"),
        uiD.lineEdit_rsyncPath->text(), tr("Executable Files (*.exe)"));
  ...
  case 1:
    newLineEdit = QFileDialog::getOpenFileName(
        this, tr("Select ssh command", "file selection dialog title"),
        uiD.lineEdit_sshPath->text(), tr("Executable Files (*.exe)"));
    if (newLineEdit != "") // if something is selected indeed
  ...
  case 2:
    newLineEdit = QFileDialog::getOpenFileName(
        this, tr("Select cygpath command", "file selection dialog title"),
        uiD.lineEdit_cygpath->text(), tr("Executable Files (*.exe)"));
    if (newLineEdit != "") // if something is selected indeed
  ...
  case 3:
    newLineEdit = QFileDialog::getOpenFileName(
        this, tr("Select dosdev command", "file selection dialog title"),
        uiD.lineEdit_dosdev->text(), tr("Executable Files (*.exe)"));
    if (newLineEdit != "") // if something is selected indeed
  ...
  case 5:
    fd =
        new QDirDialog("", uiD.lineEdit_tempdir->text(), this, "Dir selection");
...
void winDialog::useDefault(const int type) {
  ...
  case 0:
    if (WINrunning)
      uiD.lineEdit_rsyncPath->setText(
          rsyncDefaultWinCommand); // Default rsync path for windows (declared
                                   // in global.h)
    if (OS2running)
      uiD.lineEdit_rsyncPath->setText(
          rsyncDefaultCommand); // Default rsync path for OS2 (declared in
                                // global.h)
    break;
  // ssh default button
  case 1:
    if (WINrunning)
      uiD.lineEdit_sshPath->setText(
          sshDefaultWinCommand); // Default ssh path for windows (declared in
                                 // global.h)
    if (OS2running)
      uiD.lineEdit_sshPath->setText(
          sshDefaultCommand); // Default ssh path for OS2 (declared in global.h)
    break;
  case 2:
