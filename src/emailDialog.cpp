...
emailDialog::emailDialog(QDialog *parent) : QDialog(parent) {
  ...
  connect(uiE.lineEdit_command, SIGNAL(textEdited(QString)), this,
          SLOT(enableTLS()));
  connect(uiE.checkBox_disable_tls, SIGNAL(stateChanged(int)), this,
          SLOT(enableTLS()));
  ...
  connect(uiE.toolButton_default_command, SIGNAL(clicked()), signalMapper1,
          SLOT(map()));
  signalMapper1->setMapping(uiE.toolButton_default_subject, 1);
  connect(uiE.toolButton_default_subject, SIGNAL(clicked()), signalMapper1,
          SLOT(map()));
  signalMapper1->setMapping(uiE.toolButton_default_body, 2);
  connect(uiE.toolButton_default_body, SIGNAL(clicked()), signalMapper1,
          SLOT(map()));
  connect(signalMapper1, SIGNAL(mapped(const int)), this,
          SLOT(revertDefault(const int)));
...
// -------------------------------- closeEvent emitted
// --------------------------------
void emailDialog::closeEvent(QCloseEvent *event) { event->accept(); }
...
bool emailDialog::checkFields() {
  bool returnVal = true;
  if (uiE.checkBox_conditionNever
          ->isChecked()) // if the never condition is checked, just return. No
  ...
  /* This is commented because a command with no arguments (eg a script) is
  possible to be used if (uiE.lineEdit_arguments -> text() == "")
  {
...
void emailDialog::fillVariables() {
  ...
  if (WINrunning) {
    ...
    // QString smtpUser = uiE.lineEdit_arg_smtp_user->text();    // security
    // issues ? QString smtpPass = uiE.lineEdit_arg_smtp_pass->text();    //
    // security issues ?
  } else
...
void emailDialog::emailTest() {
  ...
  if (!checkFields()) {
    textDialog textdialogW("QtWarning", warningMessage, this);
    textdialogW.exec();
    return; // do not proceed if something is wrong with the fields (eg command
            // is empty)
  }
...
int emailDialog::getGoOn() { return GoOn; }
