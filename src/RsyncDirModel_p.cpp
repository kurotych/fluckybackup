/****************************************************************************
    ...
    This file is developed by VCreate Logic Private Limited and used as part of
the "luckyBackup" project It is distributed under the terms of the GNU General
Public License It is free software: you can redistribute it and/or modify it
...
later version.
    ...
    along with the luckyBackup project.  If not, see
...
struct RsyncEntryData {
  RsyncEntryData() : parent(0), childrenFetched(false), fetching(false) {}
...
RsyncEntry *RsyncEntry::parentEntry() const { return d->parent; }

QList<RsyncEntry *> RsyncEntry::childEntries() const { return d->children; }

bool RsyncEntry::isChildrenFetched() const { return d->childrenFetched; }

bool RsyncEntry::isFetching() const { return d->fetching; }
...
QUrlInfo RsyncEntry::urlInfo() const { return d->urlInfo; }

QIcon RsyncEntry::icon() const { return d->icon; }
...
void RsyncEntry::clear() { d->children.clear(); }
...
struct RsyncDirListerOperationData {
  RsyncDirListerOperationData()
      : parentEntry(0), executing(false), finished(false), success(false),
        RsyncCmdId(0) {}
...
RsyncDirListerOperation::RsyncDirListerOperation(RsyncDirLister *parent)
  ...
  connect(d->syncProcess, SIGNAL(finished(int, QProcess::ExitStatus)), this,
          SLOT(procFinished()));
  connect(d->syncProcess, SIGNAL(error(QProcess::ProcessError)), this,
          SLOT(procError()));
}
...
RsyncDirListerOperation::~RsyncDirListerOperation() { delete d; }
...
int RsyncDirListerOperation::id() const { return d->id; }

bool RsyncDirListerOperation::isExecuting() const { return d->executing; }

bool RsyncDirListerOperation::isFinished() const { return d->finished; }

bool RsyncDirListerOperation::isSuccessfullyFinished() const {
  return d->success;
}
QString RsyncDirListerOperation::path() const { return d->url.path(); }
void RsyncDirListerOperation::execute() {
  d->executing = true;
  QStringList tempArgs = d->parent->arguments();
  tempArgs << path();
  d->syncProcess->start(d->parent->command(),
                        tempArgs); // execute rsync command with rsyncArguments
...
void RsyncDirListerOperation::procFinished() {
  ...
  QTextStream outputStream(
      new QString(codec->toUnicode(d->syncProcess->readAllStandardOutput())));
  QString line;
  ...
  do {
    ...
    if (d1[0] == '.')
      continue;
    if (d->parentEntry) {
      d->parentEntry->addUrlInfo(QUrlInfo(
          d1, 777, "root", "root", data[1].toLong(),
          QDateTime::fromString(data[2] + " " + data[3], "yyyy/MM/dd HH:mm:ss"),
          QDateTime::fromString(data[2] + " " + data[3], "yyyy/MM/dd HH:mm:ss"),
          isDir, isFile, isLink, false, true, isExec));
    } else {
      parentEntries.append(new RsyncEntry(QUrlInfo(
          d1, 777, "root", "root", data[1].toLong(),
          QDateTime::fromString(data[2] + " " + data[3], "yyyy/MM/dd HH:mm:ss"),
          QDateTime::fromString(data[2] + " " + data[3], "yyyy/MM/dd HH:mm:ss"),
          isDir, isFile, isLink, false, true, isExec)));
...
struct RsyncDirListerData {
  RsyncDirListerData() : topLevelEntriesFetched(false), currentParent(0) {}
...
RsyncDirLister::RsyncDirLister(QObject *parent) : QObject(parent) {
  d = new RsyncDirListerData;
  d->command = "rsync";
  d->rsyncArguments << "--list-only";
}

RsyncDirLister::~RsyncDirLister() { delete d; }

void RsyncDirLister::setUrl(const QUrl &url) { d->url = url; }
...
QUrl RsyncDirLister::url() const { return d->url; }

QString RsyncDirLister::command() const { return d->command; }

QStringList RsyncDirLister::arguments() const { return d->rsyncArguments; }
...
QString RsyncDirLister::lastError() const { return d->errorMessage; }

void RsyncDirLister::list() {
  cleanOperations();
  // Create a list operation and queue it.
  RsyncDirListerOperation *listOper = new RsyncDirListerOperation(this);
...
void RsyncDirLister::list(RsyncEntry *entry) {
  ...
  // Create a list operation and queue it.
  RsyncDirListerOperation *listOper = new RsyncDirListerOperation(this, entry);
...
FileIconCache::~FileIconCache() { delete d; }
