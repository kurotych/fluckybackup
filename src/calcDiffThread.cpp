/* ---------------------------------- calcDiffThread.cpp
---------------------------------------------------------------------------
 Class to calculate the differences between source & snapshot in a seperate
thread
...
// class CalcDiffThread
// Destructor=================================================================================================
CalcDiffThread::~CalcDiffThread() { wait(); }
...
void CalcDiffThread::run() {
  ...
  emit updateViewer(tr("Scanning destination") + "\n\n..." + tr("Please wait"),
                    false);
  QFileInfoList returnDirContents = readRecursive(
      dirToReadString); // this is the destination as a Qfileinfo list

  int countDirEntries = 0, countReturnEntries = 0;

  // Remove the "luckybackup snapshots root" directory from the
  // returnDirContents list
  ...
  // Remove files/dirs listed at changes.file from the QFileInfo list
  // (destination)
  ...
  while ((count >= CurrentSnapshotNo) && (!syncTYPE)) {
    QString SnapshotDirectoryStringToRead =
        Operation[currentOperation]->GetSnapshotsListItem(count);
    QString SnapshotDirectoryToRead = dest + snapDefaultDir +
                                      SnapshotDirectoryStringToRead + SLASH +
                                      sourceLast;

    QDir currentsnapdir(SnapshotDirectoryToRead);
    if ((currentsnapdir.exists()) && (!syncTYPE)) {
      emit updateViewer(tr("Scanning snapshot") + " " +
                            SnapshotDirectoryStringToRead + "\n\n..." +
                            tr("Please wait"),
                        false);
      QFileInfoList currentsnapdirContents = readRecursive(
          SnapshotDirectoryToRead); // this is the snapshots directory currently
                                    // read as a QFileInfo list
      countDirEntries = 0;
      ...
      while (countDirEntries < currentsnapdirContents.size()) {
        ...
        while (countReturnEntries < returnDirContents.size()) {
          QFileInfo localFileInfo = returnDirContents.at(countReturnEntries);
          QString destFile = localFileInfo.filePath();
          for (int count3 = CurrentSnapshotNo; count3 < currentSnaps - 1;
               count3++) // if the destFile belongs to a "later" snapFile
          {
            QString IlostTrack =
                Operation[currentOperation]->GetSnapshotsListItem(count3);
            if (destFile.startsWith(dest + snapDefaultDir + IlostTrack))
              destFile.remove(dest + snapDefaultDir + IlostTrack + SLASH +
                                  sourceLast,
                              Qt::CaseSensitive);
          }
          destFile.remove(dest, Qt::CaseSensitive);
        ...
        if ((!replaced) &&
            (!removedFiles.contains(snapFile, Qt::CaseSensitive)))
  ...
  emit updateViewer(tr("Scanning source") + "\n\n..." + tr("Please wait"),
                    false);
  QFileInfoList sourceDirAsList =
      readRecursive(source); // This is source as a QFileInfo list

  QString calcInfoText =
      tr("Calculating differences") + "\n\n..." + tr("Please wait") + "\n\n";
  QString sourceDirAsListSize = countStr.setNum(sourceDirAsList.size());

  countDirEntries = 0;
  while (countDirEntries < sourceDirAsList.size()) {
    bool foundMatchSourceToSnap = false; // If a file with some filename exists
                                         // at the source and the snapshot
    ...
    emit updateViewer(calcInfoText + countStr.setNum(countDirEntries) + " / " +
                          sourceDirAsListSize,
                      false); // Give the user sth to watch counting
    if (abortNOW)
      return;
    ...
    while (countReturnEntries < returnDirContents.size()) {
      ...
      for (count = currentSnaps - 2; count >= CurrentSnapshotNo; count--) {
        QString SnapshotDirectoryStringToRead =
            Operation[currentOperation]->GetSnapshotsListItem(count);
        if (snapFile.startsWith(dest + snapDefaultDir +
                                SnapshotDirectoryStringToRead))
          snapFile.remove(dest + snapDefaultDir +
                              SnapshotDirectoryStringToRead + SLASH +
                              sourceLast,
                          Qt::CaseSensitive);
        // if (snapFile.startsWith(dest + snapDefaultDir +
        // CurrentSnapshotString) )
        //     snapFile.remove(dest + snapDefaultDir + CurrentSnapshotString +
        //     SLASH + sourceLast, Qt::CaseSensitive);
      }
      // ********************* TO EDIT
      // ***************************************************************************************
      ...
      //	The snapshot data will replace the relevant source data, at
      //RESTORE
      if (snapFile == sourceFile) {
        ...
        if ((snapFileInfo.lastModified() != sourceFileInfo.lastModified()) ||
          ...
          else if (changedfiles.size() < 10000)
            changedfiles.append(
                "* <b>" + snapFile + "</b> - " +
                snapFileInfo.lastModified().toString("yyyy MMM dd - hh:mm:ss") +
                "<br>");
    ...
    //	These will be deleted at RESTORE if "delete existing source data that do
    //not exist at the backup" is checked
    if (!foundMatchSourceToSnap) {
      existingSource.append(sourceFileInfo);
      if (sourceFileInfo.isDir())
        sourceFile.append(SLASH);
      if (existingsource.size() < 10000)
        existingsource.append(
            "* <b>" + sourceFile + "</b> - " +
            sourceFileInfo.lastModified().toString("yyyy MMM dd - hh:mm:ss") +
            "<br>");
    }
  ...
  if (syncTYPE)
    returnString.append("		<b>" +
                        tr("Sync dir A & B differences") + "</b><br>");
  else
    returnString.append("		<b>" +
                        tr("Snapshot and source differences") + "</b><br>");
  returnString.append(
  ...
  if (changedfiles != "") {
    ...
    if (syncTYPE)
      returnString.append(" " +
                          tr("Sync dir B data that also exists at sync dir A "
                             "and is different") +
                          "</p>");
    else {
      returnString.append(
          " " +
          tr("Snapshot data that also exists at the source and is different") +
          "<br>");
      returnString.append(
          " " +
          tr("The snapshot data will replace the source data during RESTORE") +
          "</p>");
    }
    returnString.append(changedfiles);
    if (changedfiles.size() > 9900)
      returnString.append(
          " <br>" + tr("Data amount is huge. Displaying only first entries") +
          "<br>");
    returnString.append(
  ...
  if (existingsnap != "") {
    ...
    if (syncTYPE)
      returnString.append(
          " " + tr("Sync dir B data that do NOT exist at sync dir A") + "</p>");
    else {
      returnString.append(
          " " + tr("Snapshot data that do NOT exist at the source") + "<br>");
      returnString.append(
          " " +
          tr("These will be transferred over at the source during RESTORE") +
          "</p>");
    }
  ...
  if (existingsource != "") {
    ...
    if (syncTYPE)
      returnString.append(
          " " + tr("Sync dir A data that do NOT exist at sync dir B") + "</p>");
    else {
      returnString.append(
          " " + tr("Source data that do NOT exist at the snapshot") + "<br>");
      returnString.append(" " +
                          tr("These will be deleted during RESTORE only if you "
                             "enable the option:") +
                          "<br>");
      returnString.append(
          " \"<b>" +
          tr("delete existing source data that do not exist at the backup") +
          "</b>\"</p>");
    }
    ...
    if (existingsource.size() > 9900)
      returnString.append(
          " <br>" + tr("Data amount is huge. Displaying only first entries") +
          "<br>");
    returnString.append("______________________________________________________"
                        "___________<br>");
  }
...
// set
// parametres=================================================================================================
void CalcDiffThread::passParams(QString dirtoreadstring, QString desti,
                                QString currentsnapshotdirectory,
                                QString currentsnapshotstring,
                                QString sourcelast, QString sourcei,
                                int currentsnapshotno, int currentsnaps,
                                bool synctype) {
...
// text=================================================================================================
QString CalcDiffThread::returnResult() { return returnString; }

// Set the abortNOW
// ===========================================================================================
void CalcDiffThread::setAbort(bool abortset) { abortNOW = abortset; }
...
QFileInfoList CalcDiffThread::readRecursive(QString dirToReadString) {
  ...
  dirToRead.setFilter(QDir::AllDirs | QDir::NoDotAndDotDot | QDir::AllEntries |
                      QDir::Hidden);
  dirToRead.setSorting(QDir::Name | QDir::DirsFirst | QDir::IgnoreCase |
                       QDir::LocaleAware);
  ...
  while (!dirStack.empty()) {
    ...
    lastDir.setFilter(QDir::AllDirs | QDir::NoDotAndDotDot | QDir::AllEntries |
                      QDir::Hidden);
    lastDir.setSorting(QDir::Name | QDir::DirsFirst | QDir::IgnoreCase |
                       QDir::LocaleAware);
