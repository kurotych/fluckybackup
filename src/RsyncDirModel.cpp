...
struct RsyncDirModelData {
  ...
  //    QStringList rsyncArguments;  // This stringList holds all arguments for
  //    the rsync command QProcess *syncProcess; bool ProcReportedError;
  RsyncDirLister *lister;
};
...
RsyncDirModel::RsyncDirModel(QObject *parent) : QAbstractItemModel(parent) {
  ...
  connect(d->lister, SIGNAL(listingDone(RsyncEntry *)), this,
          SLOT(slotListingDone(RsyncEntry *)));
  //    connect(syncProcess, SIGNAL (finished(int, QProcess::ExitStatus)), this,
  //    SLOT(procFinished())); connect(syncProcess, SIGNAL
  //    (error(QProcess::ProcessError)), this, SLOT(procError()));

  //    d->ProcReportedError = false;      // This might change as soon as
  //    syncprocess will start ()
}

RsyncDirModel::~RsyncDirModel() { delete d; }
...
QUrl RsyncDirModel::url() const { return d->lister->url(); }
...
QModelIndex RsyncDirModel::index(int row, int col,
...
QVariant RsyncDirModel::headerData(int section, Qt::Orientation orientation,
                                   int role) const {
