...
int main(int argc, char *argv[]) {
  ...
  if (!argumentsTest(argc, argv))
    return 1; // error code1: something wrong with the command arguments - app
              // fail to start (or just --help or --version given as args)

  // profile validity check for console mode or gui-no profile specified (will
  // use default)
  ...
  if (qobject_cast<QApplication *>(app.data())) // execute gui
  {
    ...
    translator_qt.load(QString("qt_") + locale,
    ...
    /*       appPath = app->applicationDirPath();  // This is used for windows
       app path. It's also causing a ...
                                                //"QCoreApplication::applicationDirPath:
       Please instantiate the QApplication object first" WARNING message
           rsyncDefaultWinCommand = appPath+"/rsync.exe"; // Holds the default
       rsync command for windows sshDefaultWinCommand = appPath+"/ssh.exe"; //
       Holds the default ssh command for windows mapdrive="w"; vshadowDir=
       appPath; vshadowDefaultDir=      appPath; dosdevCommand=
       appPath+"/dosdev.exe"; dosdevDefaultCommand=   appPath+"/dosdev.exe";
