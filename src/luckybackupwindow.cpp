...
-----------------------------------------------------------------------------
cpp file that does all stuff needed when a signal is transmitted (eg button
pressed) from the main app window
...
luckyBackupWindow::luckyBackupWindow(QMainWindow *parent)
  ...
  QStringList emptyList = emptydir.entryList(QDir::AllEntries | QDir::Hidden |
                                             QDir::NoDotAndDotDot);
  if (emptyList.size() > 0) {
    QProcess *rmProcess;
  ...
  connect(ui.pushButton_exit, SIGNAL(clicked()), this,
          SLOT(exitApp())); // pushbutton exit
  connect(ui.actionQuit, SIGNAL(triggered()), this,
          SLOT(exitApp())); // menu action quit

  connect(ui.comboBox_profile, SIGNAL(currentIndexChanged(const int)), this,
          SLOT(profileComboChanged()));

  connect(ui.actionNew, SIGNAL(triggered()), this,
          SLOT(NewProfile())); // menu action newProfile
  connect(ui.actionRename, SIGNAL(triggered()), this,
          SLOT(renameCurrentProfile())); // menu action renameProfile
  ...
  connect(ui.actionDelete, SIGNAL(triggered()), this,
          SLOT(deleteCurrentProfile())); // action delete (currentProfile)
  connect(ui.actionDefault, SIGNAL(triggered()), this,
          SLOT(setDefaultProfile())); // action default (set currentProfile as
                                      // default)
  connect(ui.actionImport, SIGNAL(triggered()), this,
          SLOT(importProfile())); // action import profile
  connect(ui.actionExport, SIGNAL(triggered()), this,
          SLOT(exportProfile())); // action export profile
  connect(ui.actionDuplicate_Profile, SIGNAL(triggered()), this,
          SLOT(duplicateProfile())); // action duplicate profile
  connect(
      ui.actionDescription, SIGNAL(triggered()), this,
      SLOT(changeProfileDescription())); // menu action changeProfileDescription

  connect(ui.action_TaskAdd, SIGNAL(triggered()), this,
          SLOT(add())); // action add task
  connect(ui.action_TaskModify, SIGNAL(triggered()), this,
          SLOT(modify())); // action modify task
  connect(ui.action_TaskRemove, SIGNAL(triggered()), this,
          SLOT(remove())); // action remove task
  connect(ui.action_TaskDuplicate, SIGNAL(triggered()), this,
          SLOT(duplicateTask())); // action create duplicate
  connect(ui.action_TaskCreateRestore, SIGNAL(triggered()), this,
          SLOT(AlsoCreateRestore())); // action create restore
  connect(ui.action_TaskManageBackup, SIGNAL(triggered()), this,
          SLOT(manage())); // action manage backup

  connect(ui.pushButton_add, SIGNAL(clicked()), this,
          SLOT(add())); // pushbutton create (task)
  connect(ui.pushButton_edit, SIGNAL(clicked()), this,
          SLOT(modify())); // pushbutton modify (task)
  connect(ui.pushButton_remove, SIGNAL(clicked()), this,
          SLOT(remove())); // pushbutton remove (task)
  connect(ui.pushButton_up, SIGNAL(clicked()), this,
          SLOT(moveUp())); // pushbutton up
  connect(ui.pushButton_down, SIGNAL(clicked()), this,
          SLOT(moveDown())); // pushbutton down
  connect(ui.pushButton_start, SIGNAL(clicked()), this,
          SLOT(start())); // pushbutton start
  // find a way to emit a signal when spacebar is used to change the state of an
  // item.  itemChanged does not seem to work !!
  connect(ui.listWidget_operations, SIGNAL(itemClicked(QListWidgetItem *)),
          this, SLOT(checkCurrentItem(QListWidgetItem *)));
  connect(ui.listWidget_operations, SIGNAL(itemChanged(QListWidgetItem *)),
  ...
  connect(ui.pushButton_previousError, SIGNAL(clicked()), this,
          SLOT(previousErrorJump())); // pushbutton jump to prev error
  connect(ui.pushButton_nextError, SIGNAL(clicked()), this,
          SLOT(nextErrorJump())); // pushbutton jump to next error

  connect(ui.pushButton_InfoCollapse, SIGNAL(clicked()), this,
          SLOT(hideShowInfoWindow())); // pushbutton hide/show info window

  // Load the default profile as soon as the app starts or the one given as a
  ...
  loadCurrentProfile(); // load the current (default or not ) profile - this is
                        // inside functions.cpp

  // *********************TESTING purposes message boxs
  // ****************************************************************************
  /*QMessageBox::information(this, "LB","CurrentProfile=" + currentProfile +
  "\ntotal tasks: " + countStr.setNum(TotalOperations)); QString preRelease="8";
  QString preDate="09 Feb 2012"; QString tempOSrunning="";if (OS2running)
  tempOSrunning="OS2";if (WINrunning) tempOSrunning="windows";if
  (!notXnixRunning) tempOSrunning="*nix"; QMessageBox::warning(this,
  "luckybackup",       "This is a testing version of <b>luckyBackup</b><br>"
                                                  "NOT to be used as an official
  release and NOT to be trusted for actual backups<br><br>" "pre-release: <b>" +
  preRelease +"</b><br>version: <b>" + appVersionString + "</b><br>date:
  <b>"+preDate+
  ...
  checkCurrentItem(
      ui.listWidget_operations
          ->currentItem()); // refresh the selected (last) task as well as the
                            // menu items that depend on its type (sync/backup)

  createProfileCombo(); // update the profile combobox with all existing
                        // profiles and set index to currentProfile
  ...
  QString currentLanguageIs =
      tr("English", "Do not translate this. Enter here the language of the "
                    "translation in native characters instead. This is how "
                    "your translation will appear under the 'language' menu.");
}
...
void luckyBackupWindow::exitApp() {
  if (NOWexecuting) // if a profile execution is in progress just minimize to
                    // tray
  {
  ...
  if (isProfileSaved() ==
      0) // if the current profile is not saved, ask the user if he/she wants to
    return;
...
void luckyBackupWindow::profileComboChanged() {
  ...
  QString ComboReadFullPath =
      profileDir + (ui.comboBox_profile->currentText()) + ".profile";
...
void luckyBackupWindow::NewProfile() {
  ...
  if (textdialog.getGoOn() == 2) {
    InfoData =
        tr("Failed to create new profile",
           "full phrase: 'Failed to create new profile <PROFILENAME>'") +
        " <b>" + profileName + "</b><br><font color=red>" +
        tr("The name you have entered contains only invalid characters") +
        "</font>";
    ui.textBrowser_info->setText(InfoData); // update the info window
    return;
  }
  ...
  {
    message =
        tr("Profile", "full phrase: 'Profile <PROFILENAME> already exists'") +
        " <b>" + newProfileName + "</b> " +
        tr("already exists",
           "full phrase: 'Profile <PROFILENAME> already exists'") +
        "<br>" + tr("Would you like to replace it ?");
    ...
    {
      if (!newprofile.remove()) // delete the old profile or display a message
                                // if you can't
      {
        InfoData =
            "<font color=red><b>" + tr("ERROR") + "</font></b><br>" +
            tr("Failed to replace profile",
               "full phrase: 'Failed to replace profile <PROFILENAME>'") +
...
void luckyBackupWindow::renameCurrentProfile() {
  ...
  if (textdialog.getGoOn() == 2) {
    InfoData = tr("Failed to rename profile",
                  "full phrase: 'Failed to rename profile <PROFILENAME>'") +
               " <b>" + profileName + "</b><br><font color=red>" +
               "The name you have entered contains only invalid characters" +
               "</font>";
    ui.textBrowser_info->setText(InfoData); // update the info window
    return;
  }

  if (textdialog.getGoOn() ==
      3) // if the new profile name is the same with the old one do nothing
  {
    InfoData.append(tr("profile", "full phrase: 'profile <PROFILENAME> renamed "
                                  "successfully to <NEWPROFILENAME>'") +
                    " <b>" + profileName + "</b><br>...<font color=green>" +
                    tr("renamed successfully to",
                       "full phrase: 'profile <PROFILENAME> renamed "
                       "successfully to <NEWPROFILENAME>'") +
  ...
  InfoData.append(" <b>" + profileName + "</b><br>...<font color=green>" +
                  tr("renamed successfully to",
                     "full phrase: '(default) profile <PROFILENAME> renamed "
                     "successfully to <NEWPROFILENAME>'") +
                  "</font> <b>" + newProfileName + "</b>");

  // update all relevant snap .changes as well as log files
  if (arrangeLogSnap(1, "rename", newProfileName))
    InfoData.append("<br><br>" +
                    tr("All relevant files (logs etc) have also been updated"));
  else
    InfoData.append("<br><br>" +
                    tr("Failed to update all relevant files (logs etc)"));
...
void luckyBackupWindow::deleteCurrentProfile() {
  ...
  if (currentProfile == defaultProfile)
    message = tr("Are you sure you want to delete the default profile",
                 "full phrase: 'Are you sure you want to delete the default "
                 "profile: <PROFILENAME> ? (a new blank one, with the same "
    ...
    message = tr("Are you sure you want to delete profile",
                 "full phrase: 'Are you sure you want to delete profile: "
                 "<PROFILENAME> ?'") +
              ": <b>" + profileName + "</b> ?";
  ...
  if (textdialogQ.getGoOn() > 0) // if user answers yes
  {
    ...
    if (!profile.remove()) {
      InfoData.append(
          ...
          tr("Failed to delete profile",
             "full phrase: 'Failed to delete profile <PROFILENAME>'") +
          ": <b>" + profileName + "</b><br><font color=red>" +
    ...
    InfoData.append(
        tr("Profile",
           "full phrase: 'Profile <PROFILENAME> deleted successfully'") +
        ": <b>" + profileName + "</b><br>...<font color=green>" +
    ...
    if (arrangeLogSnap(1, "delete", profileName))
      InfoData.append(
          "<br>" + tr("All relevant files (logs etc) have also been deleted") +
          "<br>");
    else
      InfoData.append("<br>" +
...
void luckyBackupWindow::setDefaultProfile() {
  ...
  saveSettings();
  InfoData.append(
      tr("Profile", "full phrase: 'Profile <PROFILENAME> set as default'") +
      ": <b>" + profileName + "</b><br>...<font color=green>" +
      tr("set as default",
         "full phrase: 'Profile <PROFILENAME> set as default'") +
      " !!</font><br>");
  ui.textBrowser_info->setText(InfoData); // update the info window
}
...
void luckyBackupWindow::duplicateProfile() {
  ...
  if (textdialog.getGoOn() == 2) {
    InfoData = tr("Failed to duplicate profile",
                  "full phrase: 'Failed to duplicate profile <PROFILENAME>'") +
               " <b>" + profileName + "</b><br><font color=red>" +
               "The name you have entered contains only invalid characters" +
               "</font>";
    ui.textBrowser_info->setText(InfoData); // update the info window
  ...
  if (duplicateprofile
          .exists()) // if the duplicateprofile already exists ask to replace
  {
    message =
        tr("Profile", "full phrase: 'Profile <PROFILENAME> already exists'") +
        " <b>" + duplicateProfileName + "</b> " +
        tr("already exists",
           "full phrase: 'Profile <PROFILENAME> already exists'") +
        "<br>" + tr("Would you like to replace it ?");
    textDialog textdialogQ("QtQuestion", message, this);
    ...
    if (textdialogQ.getGoOn() > 0) // if user answers yes
    {
      if (!duplicateprofile.remove()) // delete the old profile or display a
  ...
  {
    InfoData = "<font color=red><b>" + tr("ERROR") + "</font></b><br>" +
               tr("Failed to duplicate profile",
                  "full phrase: 'Failed to duplicate profile <PROFILENAME> as "
                  "<FILENAME>'") +
               " <b>" + profileName + "</b><br>" +
               tr("as", "full phrase: 'Failed to duplicate profile "
                        "<PROFILENAME> as <FILENAME>'") +
               " <b>" + duplicateProfileName + "</b><br><font color=red>" +
               profile.errorString() + "</font>";
    ui.textBrowser_info->setText(InfoData); // update the info window
  ...
  InfoData.append(tr("Profile", "full phrase: 'Profile <PROFILENAME> "
                                "duplicated successfully as <FILENAME>'") +
                  ": <b>" + profileName + "</b><br>...<font color=green>" +
                  tr("duplicated successfully as",
                     "full phrase: 'Profile <PROFILENAME> duplicated "
  ...
  if (arrangeLogSnap(1, "duplicate", duplicateProfileName))
    InfoData.append(
        "<br><br>" +
        tr("All relevant files (logs etc) have also been duplicated"));
  else
    InfoData.append("<br><br>" +
                    tr("Failed to duplicate all relevant files (logs etc)"));
...
void luckyBackupWindow::changeProfileDescription() {
  ...
  InfoData.append(tr("The description of profile",
                     "full phrase: 'The description of profile <PROFILENAME> "
                     "set to: <DESCRIPTION>'") +
                  " <b>" + profileName + "</b> " +
                  tr("set to", "full phrase: 'The description of profile "
                               "<PROFILENAME> set to: <DESCRIPTION>'") +
...
// duplicateTask
// ================================================================================================================================
// duplicates the current task (creates a new task - same as selected - and
// opens modify dialog)
void luckyBackupWindow::duplicateTask() {
  ...
  currentOperation =
      ui.listWidget_operations->currentRow(); // current task list row number
  ...
  TotalOperations =
      ui.listWidget_operations->count(); // Get the Operations list size
  if ((TotalOperations) >=
      (maxOperations - 1)) // if the maxOperations limit-1 is reached
  {
    textDialog textdialogW(
        "QtWarning",
        tr("You have reached the maximum limit of supported tasks") +
            " <br><br>" +
  ...
  *tempOp = *Operation[oldCurrentOperation];
  ...
  Operation[currentOperation]->SetName(
      (Operation[currentOperation]->GetName()) + " DUPLICATE");
  ...
  TotalOperations = ui.listWidget_operations
                        ->count(); // set the TotalOperations to what it is now

  saveOrNot = false;
  modify(); // open the modify dialog for the new task, false == do not save the
            // result
  saveOrNot = true;
  ...
  if (modifyOK) {
    ...
    ui.listWidget_operations->takeItem(
        currentOperation); // take out the selected item from the list
    TotalOperations =
        ui.listWidget_operations
            ->count(); // TotalOperations takes the new value (old - 1)
  }
}
...
void luckyBackupWindow::AlsoCreateRestore() {
  ...
  *tempOp = *Operation[oldCurrentOperation];
  ...
  // radiobuttons
  Operation[currentOperation]->SetName(
      "RESTORE - " + (Operation[currentOperation]->GetName()));
  ...
  Operation[currentOperation]->SetRemoteDestination(
      Operation[oldCurrentOperation]->GetRemoteSource());
  Operation[currentOperation]->SetRemoteSource(
  ...
  if (Operation[currentOperation]->GetTypeDirName()) {
    if (WINrunning && Operation[currentOperation]->GetRemote()) {
      sourceDir.append(
          destDir.right(destDir.size() - destDir.lastIndexOf(XnixSLASH) - 1));
      destDir.chop(destDir.size() - destDir.lastIndexOf(XnixSLASH) - 1);
    } else {
      sourceDir.append(
          destDir.right(destDir.size() - destDir.lastIndexOf(SLASH) - 1));
  ...
  textDialog textdialogW(
      "QtWarning",
      tr("You have asked for a RESTORE task to be created",
         "'create restore' message line 1/4") +
          "<br><br>" +
          tr("Please have a good look at the following task properties dialog",
             "'create restore' message line 2/4") +
          " <br>" +
          tr("and change anything you wish before proceeding",
             "'create restore' message line 3/4") +
          " <br><br>" +
          tr("Please check 'Task name', 'source', 'destination' and especially "
             "the 'Delete files on the destination'' & 'skip newer destination "
             "files' check-boxes (advanced)",
             "'create restore' message line 4/4") +
          "<br>",
      this);
  textdialogW.exec();
  ...
  saveOrNot = false;
  modify(); // open the modify dialog for the new operation to make sure
            // everything is ok, false == do not save the result
  saveOrNot = true;

  if (modifyOK) // connect the operation with its RESTORE duplicate
  {
    Operation[currentOperation]->SetConnectRestore(
        Operation[oldCurrentOperation]->GetName());
    Operation[oldCurrentOperation]->SetConnectRestore(
...
void luckyBackupWindow::exportProfile() {
  ...
  QString exportProfileName =
      QFileDialog::getExistingDirectory(
          this,
          appName + " - " +
              tr("Select export directory",
                 "this is a dialog title for selecting a directory to export a "
                 "profile to"),
          SLASH, QFileDialog::ShowDirsOnly | QFileDialog::DontResolveSymlinks) +
      SLASH + profileName;

  if (exportProfileName ==
      "") // if cancel is pressed at the file dialog, do nothing
    return;
  ...
  if (!exportFullProfile(
          exportProfileName,
          "ExportFullProfile")) // try to save/export the new profile
  {
    InfoData =
        "<font color=red><b>" + tr("ERROR") + "</font></b><br>" +
        tr("Failed to export profile", "full phrase: 'Failed to export profile "
                                       "<PROFILENAME> to <DIRNAME>'") +
        " <b>" + profileName + "</b><br>" +
        tr("to", "full phrase: 'Failed to export profile <PROFILENAME> to "
                 "<DIRNAME>'") +
  ...
  InfoData.append(
      tr("Profile", "full phrase: 'Profile <PROFILENAME> exported successfully "
                    "to <DIRNAME>'") +
      ": <b>" + profileName + "</b><br>...<font color=green>" +
      tr("exported successfully to", "full phrase: 'Profile <PROFILENAME> "
                                     "exported successfully to <DIRNAME>'") +
      "</font> <b>" + exportProfileName + "</b>");

  ui.textBrowser_info->setText(InfoData); // update the info window
  setCurrentProfile(currentProfile);      // reset all currentProfile variables
  createProfileCombo(); // update the profile combobox (in case profile was
                        // exported at ~/.luckyBackup/profiles) and set index to
                        // currentProfile
}
...
void luckyBackupWindow::importProfile() {
  ...
  // This holds the full path of the profile to get imported directory (eg
  // /media/exported/default.profile)
  QString importProfilePath = QFileDialog::getExistingDirectory(
      this, appName + " - " + tr("Import Profile", "this is a dialog title"),
      SLASH, QFileDialog::ShowDirsOnly | QFileDialog::DontResolveSymlinks);
  ...
  if (!importPprofileContents.contains(
          "profiles")) // check if there exists a "profile" dir
  {
    InfoData = tr("Failed to import profile",
                  "full phrase: 'Failed to import profile <PROFILENAME>'") +
               " <b>" + importProfilePath + "</b><br><font color=red>" +
               tr("The directory you have selected does not contain valid "
                  "profile data") +
               "</font>";
    ui.textBrowser_info->setText(InfoData); // update the info window
    return;
  ...
  QString newProfileName =
      importProfilePath; //  newProfileName holds the final name of the profile
                         //  that is going to get imported (eg "default2")
  newProfileName = newProfileName.right(newProfileName.size() -
                                        newProfileName.lastIndexOf(SLASH) - 1);
  if (newProfileName.endsWith(".profile"))
    newProfileName.chop(8);
  QString importProfileName =
      newProfileName; // This holds the name of the profile to import (eg
                      // "default")

  // newProfile holds the final full path of the .profile file (eg
  // ~/.luckyBackup/profiles/default2.profile)
  QString newProfile = profileDir + newProfileName + ".profile";
  QFile newprofile;
  newprofile.setFileName(newProfile);
  ...
  while (
      newprofile
          .exists()) // if the newprofile already exists ask to rename or cancel
  {
  ...
  int ExitCodeImportProfile =
      importFullProfile(importProfilePath, newProfileName);
  // QMessageBox::information(this, "LB","Exit code=" +
  // countStr.setNum(ExitCodeImportProfile));  // TESTING
  //  if ExitCodeImportProfile != 1
  if ((ExitCodeImportProfile == 0) || (ExitCodeImportProfile == 30) ||
      (ExitCodeImportProfile == 20) || (ExitCodeImportProfile == 10)) {
    createProfileCombo();
    InfoData.append(
        tr("Profile",
           "full phrase: 'Profile <PROFILENAME> imported successfully as "
           "<NEWPROFILENAME> and added at the profile list'") +
        ": <b>" + importProfileName + "</b><br>...<font color=green>" +
        tr("imported successfully",
           "full phrase: 'Profile <PROFILENAME> imported successfully as "
           "<NEWPROFILENAME> and added at the profile list'") +
        "</font> " +
        tr("as", "full phrase: 'Profile <PROFILENAME> imported successfully as "
                 "<NEWPROFILENAME> and added at the profile list'") +
    ...
    if (ExitCodeImportProfile == 30)
      InfoData.append(
          "<br><font color=red>" +
          tr("Errors occurred during snapshots and logfiles transfer") +
          "</font>");
    if (ExitCodeImportProfile == 20)
      InfoData.append("<br><font color=red>" +
                      tr("Errors occurred during snapshots transfer") +
                      "</font>");
    if (ExitCodeImportProfile == 10)
      InfoData.append("<br><font color=red>" +
                      tr("Errors occurred during logfiles transfer") +
                      "</font>");
  } else // ExitCodeImportProfile == 1
    InfoData = tr("Failed to import profile",
                  "full phrase: 'Failed to import profile <PROFILENAME>'") +
               " <b>" + importProfileName + "</b><br><font color=red>" +
               tr("Errors occurred during the profile file transfer") +
               "</font>";
  ui.textBrowser_info->setText(InfoData); // update the info window
}
...
void luckyBackupWindow::setLanguage(QAction *action) {
  ...
  QStringList transContQm =
      transDir.entryList(QStringList("luckybackup*.qm"), QDir::Files,
                         QDir::NoSort); // list of .qm files inside transdir
  if ((transDir.exists()) && (!transContQm.isEmpty()))
    appTranslator.load(QString("luckybackup_") + currentLocale,
                       relativeTransDir);
  else
    appTranslator.load(QString("luckybackup_") + currentLocale, systemTransDir);

  translator_qt.load(QString("qt_") + currentLocale,
...
void luckyBackupWindow::hideShowInfoWindow() {
  ...
  if (IsVisibleInfoWindow) {
    ui.pushButton_InfoCollapse->setToolTip(
        tr("Hide information window",
           "hide/show information window button tooltip"));
...
// This is a modeless dialog so that users can interact with it together with
// the main app
void luckyBackupWindow::help() {
  if (helpURL.toString() == "Does_not_exist") {
    InfoData = "<font color=red><b>" + tr("ERROR") + "</font></b><br>" +
               tr("Could not locate the file",
                  "full phrase: 'Could not locate the file <FILENAME>'") +
               ": <b>" + relativeManual + "</b><br>	:-(";
    ui.textBrowser_info->setText(InfoData); // update the info window
    return;
  }
  ...
  helpbrowser->scrollToChapter(
      manualChapter); // The manualChapter variable will scroll to the relevant
                      // chapter
...
void luckyBackupWindow::add() {
  ...
  // open the modify dialog and create "Operation array" item at position
  // currentOperation
...
void luckyBackupWindow::modify() {
  ...
  currentOperation = ui.listWidget_operations
                         ->currentRow(); // current operations list row number
  if (currentOperation < 0)              // if nothing is selected do nothing
  {
  ...
  if ((tempConnect != "") &&
      (!modifyConnected)) // the operations is connected and is a not new
                          // RESTORE operation
    modifyConnected = true;
  int connectPosition = 0;
  if (tempConnect != "") // if this op is already connected with another op,
                         // (pop up a message & )hold the connection's position
  {
    count = 0;
    while (count < TotalOperations) { // get the connected operation position at
                                      // connectPosition
      if (Operation[count]->GetName() == tempConnect)
        connectPosition = count;
      count++;
    }
  }

  QString oldTaskName =
      Operation[currentOperation]->GetName(); // the original task name wil
                                              // become handy in case it changes

  // open the modify dialog and modify "Operation array" item at position
  // currentOperation
  modifyDialog modifydialog(currentOperation);
  modifydialog.exec();
  ...
  {
    Qt::CheckState isIncluded =
        ui.listWidget_operations->currentItem()
            ->checkState(); // get the include checkbox state
    QString newTaskName =
        Operation[currentOperation]->GetName(); // the new task name
    ui.listWidget_operations->takeItem(
        currentOperation); // take out the selected item from the list
                           // insert a new one at the same position with the new
                           // name
    ui.listWidget_operations->insertItem(currentOperation, newTaskName);
    ui.listWidget_operations->setCurrentRow(
        currentOperation); // highlight this row
    ui.listWidget_operations->currentItem()->setCheckState(
        isIncluded); // set the include checkbox as it was
    Operation[currentOperation]->SetConnectRestore(
        tempConnect); // set the connection to what it was
    if (tempConnect != "")
      Operation[connectPosition]->SetConnectRestore(newTaskName);
    checkCurrentItem(ui.listWidget_operations->currentItem());

    // update all relevant snap .changes as well as log files if the task name
    // changes
...
void luckyBackupWindow::remove() {
  currentOperation = ui.listWidget_operations
                         ->currentRow(); // current Included list row number
  if (currentOperation < 0)              // if nothing is selected do nothing
  {
  ...
  textDialog textdialogQ(
      "QtQuestion",
      tr("Are you sure you want to remove the following task ??") + " <br>" +
  ...
  if (textdialogQ.getGoOn() > 0) // if user answers yes
  {
    textDialog textdialogQ2("QtQuestion",
                            tr("Would you like to also remove:") + " <br>" +
                                tempConnect + "<br>??",
                            this);
    if (tempConnect != "") // if this op is already connected with another op
                           // ask to remove that too
      textdialogQ2.exec();
    ...
    while (currentOperation <
           TotalOperations) // shift array "Operation" one position up starting
                            // at current row
    {
      Operation[currentOperation] =
          Operation[currentOperation + 1]; // make the current array item the
...
void luckyBackupWindow::manage() {
  ...
  checkCurrentItem(
      ui.listWidget_operations->currentItem()); // refresh the selected task
...
void luckyBackupWindow::moveUp() {
  currentOperation =
      ui.listWidget_operations->currentRow(); // current list row number
  TotalOperations =
      ui.listWidget_operations->count(); // total number of list items
  ...
  if (currentOperation > 0) {
    ...
    QListWidgetItem *currentItem =
        ui.listWidget_operations->takeItem(currentOperation); // swap list items
...
void luckyBackupWindow::start() {
  TotalOperations = ui.listWidget_operations
                        ->count(); // set TotalOperations to what it is now

  if (!checkOperationList()) // check the Operations list for "errors" (eg
                             // nothing is included or the list is empty)
    return;

  checkDeclared(); // Check if the declared data are ok by calling
                   // checkBackupDirs, checkSyncDirs accordingly

  if (NothingToDo ==
      true) // if there is nothing to Do anyway then just display a message
  {
    InfoData =
        "<font color=red><b>" + tr("ERROR") + "</b></font><br>" +
        "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~<br><b>" +
        tr("You have only included tasks that are going to be skipped") +
        "</b><br>" + "..." + tr("nothing to do") + " !!";
  ...
  DryRun =
      ui.checkBox_DryRun->isChecked(); // set bool Dryrun according to checkbox
...
void luckyBackupWindow::schedule() {
  ...
  if (crontabUpdated)
    InfoData = tr("current crontab <font color=green>updated successfully") +
               "</font>";
  ui.textBrowser_info->setText(InfoData);
...
void luckyBackupWindow::email() {
  ...
  {
    InfoData = tr("Email preferences have been <font color=green>updated "
                  "successfully") +
               "</font>";
    ui.textBrowser_info->setText(InfoData);
...
void luckyBackupWindow::taskStateChanged() {
  ...
  // when an item is just clicked functions called: checkCurrentItem ->
  // taskStateChanged when a checkbox state changes functiond called:
  // taskStateChanged -> checkCurrentItem -> taskStateChanged when a checkbox
  // state changes not by clicking but with SPACEBAR, only taskStateChanged is
  // called !!
...
void luckyBackupWindow::checkCurrentItem(
    QListWidgetItem *thisIsTheCurrentItem) {
  taskClicked = true; // this is used at taskStateChanged() to determine if a
  ...
  {
    ui.action_TaskCreateRestore->setEnabled(
        false); // grey out the "create restore task" from menu
    ui.action_TaskManageBackup->setText(
        tr("Manage Task", "This is a top 'Task' menu action"));
    ui.action_TaskManageBackup->setToolTip(
        tr("display information regarding highlighted task",
           "This is a top 'Task' menu action tooltip"));
  } else {
    ui.action_TaskCreateRestore->setEnabled(true);
    ui.action_TaskManageBackup->setText(
        tr("Manage &Backup", "This is a top 'Task' menu action"));
    ui.action_TaskManageBackup->setToolTip(
        tr("display - restore - delete existing backups of highlighted task",
           "This is a top 'Task' menu action tooltip"));
  }

  if (ui.listWidget_operations->currentItem()->checkState() ==
      Qt::Unchecked) // if the operation is " NOT included" do not check it
  {
    ui.listWidget_operations->currentItem()->setIcon(
        QIcon(":/luckyPrefix/about.png"));
    CheckedData.append(tr("status") + ": <font color=blue><b>" +
                       tr("NOT INCLUDED") + "</font></b><br>");
    CheckedData.append(
        tr("Checks are disabled. This task will be <b>skipped</b>"));
    ui.textBrowser_info->setText(CheckedData);
    Operation[currentOperation]->SetIncluded(
        false); // also set its include state to false
    return;
  }

  Operation[currentOperation]->SetIncluded(
      true); // also set its include state to true
  ...
  if (Operation[currentOperation]->GetOK())
    ui.listWidget_operations->currentItem()->setIcon(
        QIcon(":/luckyPrefix/okay.png"));
  if (Operation[currentOperation]->GetWARNING())
    ui.listWidget_operations->currentItem()->setIcon(
        QIcon(":/luckyPrefix/cancel.png"));
  if (Operation[currentOperation]->GetCRITICAL())
    ui.listWidget_operations->currentItem()->setIcon(
        QIcon(":/luckyPrefix/warning.png"));
...
void luckyBackupWindow::refreshList() {
  ...
  createProfileCombo(); // update the profile combobox with all existing
                        // profiles and set index to currentProfile
  InfoData = tr("Task list of profile",
                "full phrase: 'Task list of profile <PROFILENAME> and "
                "'profile' drop-down list refreshed'") +
             " <b>" + profileName + "</b><br>" +
             tr("and 'profile' drop-down list",
                "full phrase: 'Task list of profile <PROFILENAME> and "
                "'profile' drop-down list refreshed'") +
             "<br>...<font color=blue>" +
             tr("refreshed", "full phrase: 'Task list of profile <PROFILENAME> "
                             "and 'profile' drop-down list refreshed'") +
             " !!</font>";
  ui.textBrowser_info->setText(InfoData);
}
