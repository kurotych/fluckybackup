/* ---------------------------------- textDialog.cpp
-------------------------------------------------------------------------- Class
to display a simple go-noGo dialog
...
// class textDialog
// Constructor=================================================================================================
// Displays a simple dialog with text info, ok-cancel buttons and a lineEdit for
// user input
textDialog::textDialog(const QString type, const QString ReplaceInput,
                       QWidget *parent)
    : QDialog(parent) {
  uiT.setupUi(this);
  ...
  if (type == "ImportProfile") {
    uiT.label_message->setText(
        tr("A profile with the same name already exists") + "<br>" +
  ...
  if (type == "CloneWarning") {
    ...
    uiT.lineEdit_userInput->setVisible(false);
    uiT.label_message->setText(
        tr("You have selected to \"Backup the contents of the source "
           "directory\"<br>together with the \"delete files on the "
           "destination\" option") +
        "<br><br>" +
        tr("BEWARE, if your intention is NOT to clone source to destination "
           "!!") +
        "<br><br>" + tr("Are you sure you wish to proceed ??") + "<br><br>");
    resize(sizeHint());
  }
...
void textDialog::cancel() {
  ...
  if (Type == "ValidateDialog") {
    ...
    TextPassed = TextPassed.right(
        TextPassed.size() -
        TextPassed.indexOf("<font color=red>")); // remove all irrelevant text
    source = TextPassed.right(
        TextPassed.size() -
        TextPassed.indexOf("<font color=magenta>")); // copy source & dest
                                                     // inside QString source
    TextPassed.remove(source); // remove source & dest from whole command
    dest = source.right(
        source.size() -
        source.lastIndexOf(
            "<font color=magenta>")); // copy dest inside QString dest
...
void textDialog::closeEvent(QCloseEvent *event) { event->accept(); }
// --------------------------------accessors-----------------------------------
int textDialog::getGoOn() { return GoOn; }
QString textDialog::getUserInput() { return userInput; }
