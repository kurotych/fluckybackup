...
logDialog::logDialog(QUrl logUrl, QWidget *parent) : QDialog(parent) {
  ...
  connect(readlogfile, SIGNAL(error(const QString)), this,
          SLOT(refreshWindowError(const QString &)));
  connect(readlogfile, SIGNAL(newText(const QString)), this,
          SLOT(refreshWindow(const QString &)));
  connect(readlogfile, SIGNAL(terminated()), this, SLOT(logReadEnd()));
  connect(readlogfile, SIGNAL(finished()), this, SLOT(logReadEnd()));

  (uiL.logView->document())
      ->setMaximumBlockCount(50000); // set maximum blocks to avoid freeze

  connect(uiL.button_close, SIGNAL(clicked()), this,
          SLOT(closeLog())); // connect close pushButton SLOT ----------------
  connect(uiL.button_previous, SIGNAL(clicked()), this,
          SLOT(prevError())); // connect previous pushButton SLOT
  connect(uiL.button_next, SIGNAL(clicked()), this,
          SLOT(nextError())); // connect next pushButton SLOT

  readlogfile->setLogUrl(logUrl.toString());
  QTimer::singleShot(
      500, this,
      SLOT(readLogFileNOW())); // allow some time for the dialog to appear
}
...
void logDialog::prevError() {
  ...
  if (errorCount <
      totalErrors - 1) // if the current error is less than the last one within
...
void logDialog::nextError() {
  ...
  if (errorCount == totalErrors - 1) // If the current error is the last within
  ...
  uiL.logView->scrollToAnchor(
      "error" + countStr.setNum(errorCount + 1)); // scroll to the current error
}
