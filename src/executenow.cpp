/* ---------------------------------- executenow.cpp
--------------------------------------------------------------------------- do
everything that deals with commands (rsync & others) execution
...
void luckyBackupWindow::executeNOW() {
  ...
  (ui.rsyncOutput->document())
      ->setMaximumBlockCount(50000); // set the maximum block count for the
                                     // output window to avoid freezing
  ...
  outputString = "";
  outputError = "";
  calculating = false; // these 3 bools are used to diplay progress of rsync at
                       // the info window
  transferring = false;
  deleting = false;
  ExecuteBefore = false;
  ...
  trayMessage =
      tr("execution of profile:",
         "tray message - execution of profile: <PROFILENAME> in progress...") +
      " " + profileName + " " +
      tr("in progress...",
         "tray message - execution of profile: <PROFILENAME> in progress...");
  if (DryRun)
  ...
  if ((QSystemTrayIcon::isSystemTrayAvailable()) &&
    ...
    if (KDErunning) {
      QProcess *dialogProcess;
      dialogProcess = new QProcess(this);
      QStringList dialogArgs;
      dialogArgs << "--title" << appName << "--passivepopup" << trayMessage
                 << "10";
      dialogProcess->start("kdialog", dialogArgs);
    } else
      LBtray->showMessage(appName, trayMessage, QSystemTrayIcon::Information,
                          3000);
  }
  ...
  connect(
      ui.AbortButton, SIGNAL(clicked()), this,
      SLOT(abortPressed())); // connect abort pushButton SLOT ----------------
  connect(ui.DoneButton, SIGNAL(clicked()), this,
          SLOT(donePressed())); // connect done pushButton SLOT ----------------
  ...
  /*    if (WINrunning)           // disable VSS until...
      {
        vssTimer= new QTimer(this);
          ...
          pipeVssFile =  new
  QFile(tempDirPath+SLASH+"qt_tempvss"+QString::number(qrand() % (999998) + 1));
  ...
  // connect(pipeVssFile,SIGNAL(readyRead()),this,SLOT(appendRsyncVssOutput()));
          ...
          pipeVssErrFile =  new
  QFile(tempDirPath+SLASH+"qt_tempvsserr"+QString::number(qrand() % (999998) +
  1)); if (pipeVssErrFile->open(QIODevice::ReadWrite)){ pipeVssErrFile->close();
  ...
  connect(pipeVssErrFile,SIGNAL(readyRead()),this,SLOT(appendRsyncVssOutput()));
          ...
          {
              connect
  (vssTimer,SIGNAL(timeout()),this,SLOT(appendRsyncVssOutput()));
              vssTimer->start(vssSleepTime);
          }
      }*/

  syncProcess =
      new QProcess(this); // create a new qprocess (for rsync) & connect signals
  connect(syncProcess, SIGNAL(readyReadStandardError()), this,
          SLOT(appendRsyncOutput()));
  connect(syncProcess, SIGNAL(readyReadStandardOutput()), this,
          SLOT(appendRsyncOutput()));
  connect(syncProcess, SIGNAL(finished(int, QProcess::ExitStatus)), this,
          SLOT(procFinished()));
  connect(syncProcess, SIGNAL(error(QProcess::ProcessError)), this,
          SLOT(procError()));
  ...
  // increase currentOperation until first operation to be executed (execute
  // tasks with a "by-pass WARNING option too)
  while ((currentOperation < TotalOperations) &&
         (!Operation[currentOperation]->GetPerform()))
    currentOperation++;
...
void luckyBackupWindow::swapGUI(QString GUImode) {
  if (GUImode == "execution") // change gui to execution mode
  {
    ...
    ui.frame_operations->setToolTip(
        tr("rsync and pre/post task commands output is displayed here",
           "tooltip"));
    ...
    if (!runImmediately)
      IsVisibleProfileToolbar =
          profileToolbar
              ->isVisible(); // hold the visibility state of this toolbar
  ...
  } else // change gui to normal mode !!
  {
    ...
    ui.frame_operations->setToolTip(
        tr("List of all the available tasks", "task list tooltip - line1") +
        "\n" +
        tr("Use the 'include checkboxes' to include or not a selected task",
           "task list tooltip - line2"));
    ...
    ui.pushButton_exit->setVisible(false); // set this to true to make the exit
                                           // button visible at normal mode
...
void luckyBackupWindow::donePressed() {
  ...
  if (QSystemTrayIcon::isSystemTrayAvailable()) // this prevents a segfault when
  ...
  InfoData = QDateTime::currentDateTime().toString(Qt::DefaultLocaleLongDate) +
             "<br>" +
             tr("Execution of profile",
                "full phrase: 'Execution of profile <PROFILENAME> finished'") +
             " <b>" + profileName + "</b> " +
             tr("finished",
                "full phrase: 'Execution of profile <PROFILENAME> finished'") +
             " !!<br>";
  if (DryRun)
    ...
    if (!savedProfile)
      InfoData.append(tr("Could not update last execution time of tasks") +
                      "<br>" + profile.errorString());
    else
...
void luckyBackupWindow::abortPressed() {
  // if (syncProcess->state() == QProcess::NotRunning)	//if syncProcess is not
  // Running (done pressed)

  // the next condition is used because for some wierd reason the abort button
  // is presses multiple times if you launch LB, run a profile twice and abort
  // both of them. The second run you abort, will emitt 2 abort clicked()
  // signals !!!
  if (ABORTpressed)
    return;
  ...
  ui.rsyncOutput->append(
      ...
      tr("Aborting: Please wait for all processes to be killed") +
      "...</font>");
  ExecuteBefore = false;
  ExecuteAfter = false;
  ABORTpressed = true;
...
void luckyBackupWindow::createTrayIcon() {
  ...
  actionAbort =
      new QAction(QIcon(":/luckyPrefix/abort.png"), tr("&Abort"), this);
  minimizeToTray =
      new QAction(QIcon(":/luckyPrefix/window_minimize.png"),
                  tr("&Minimize to tray", "tray menu action"), this);
  restoreFromTray = new QAction(QIcon(":/luckyPrefix/window_restore.png"),
                                tr("&Restore", "tray menu action"), this);

  connect(actionAbort, SIGNAL(triggered()), this,
          SLOT(abortPressed())); // tray icon action ABORT
  connect(minimizeToTray, SIGNAL(triggered()), this,
          SLOT(minimizeTray())); // tray icon action minimize to Tray
  connect(restoreFromTray, SIGNAL(triggered()), this,
          SLOT(restoreTray())); // tray icon action restore from tray
  ...
  connect(LBtray, SIGNAL(activated(QSystemTrayIcon::ActivationReason)), this,
          SLOT(LBtrayActivated(QSystemTrayIcon::ActivationReason)));
}
...
void luckyBackupWindow::LBtrayActivated(
...
void luckyBackupWindow::executeBeforeTask() {
  ...
  repeatOnFailMax =
      Operation[currentOperation]
          ->GetRepeatOnFail(); // This holds the number of re-runs to try if the
                               // task fails for a reason

  // logfile & older snapshots actions if real run is performed
  if ((!DryRun) && (currentBefore == 0) && (repeatOnFailTry == 0)) {
    int maxSnaps =
        Operation[currentOperation]
            ->GetKeepSnapshots(); // this is the number of snapshots to keep
    if (maxSnaps < 1)
      maxSnaps = 1;
    int currentSnaps =
        Operation[currentOperation]
    ...
    // first remove the older logfiles and snapshots if max keep snapshots is
    // reached
    ...
    // SNAPSHOTS REMOVAL - This is outside the (currentSnaps >= maxSnaps)
    // condition, because it will eventually cause the snapshots directory to
    // get created so that the profile will be backed up without problems later
    // on First calculate the folder where snapshots go (tempDestination)
    ...
    if (RemoteDestUsed) {
      ...
      if (Operation[currentOperation]->GetRemoteModule() &&
        ...
        remoteArgs.append("--password-file=" +
                          (Operation[currentOperation]->GetRemotePassword()));
    ...
    if (currentSnaps >= maxSnaps) {
      outputInsert =
          "\n<font color=magenta>" +
          tr("Removing old snapshots and logfiles of task",
             "info message displayed during ...data removal\nFull phrase: "
             "Removing old snapshots and logfiles of task: <TASKNAME>") +
      ...
      // increase the remove limit to include the source.size() if "backup dir
      // by name" is used
      ...
      // we will delete the snapshots directory by using an rsync command with
      // an empty source:
      QProcess *rmProcess;
      ...
      while (snapToKeep < currentSnaps) {
        if (WINrunning && RemoteDestUsed)
          rmArgs.append(
              "--filter=protect " +
              Operation[currentOperation]->GetSnapshotsListItem(snapToKeep) +
              XnixSLASH);
        else
          rmArgs.append(
              "--filter=protect " +
      ...
      if (WINrunning && RemoteDestUsed)
        rmArgs.append("--filter=protect " + profileName + ".profile" +
                      XnixSLASH);
      else
      ...
      if (RemoteDestUsed)
        // rmArgs.append(remoteArgs);  // use operator << instead of append to
        // maintain compatiiblity with debian 5
        rmArgs << remoteArgs;
      ...
      if (WINrunning) {
        ...
        // bool createWinRsyncCommand(tempDirPath,QFile command1,QFile
        // command2,bool vss,QString rsyncArgs,QString source,QString dest);
      ...
      if ((rmProcess->exitCode()) == 0)
        ui.rsyncOutput->append(
            "\n" +
            tr("Removed all older snapshots data")); // +" " + tempDestination +
                                                     // Operation[currentOperation]
                                                     // ->
      ...
      while (count < (currentSnaps - maxSnaps + 1)) {
        ...
        // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        snapchangesfilename =
            snapChangesDir + profileName + "-" +
            Operation[currentOperation]->GetName() + "-" +
            (Operation[currentOperation]->GetSnapshotsListItem(0)) +
            ".changes.log";
        ...
        // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        logfilename = logDir + profileName + "-" +
                      Operation[currentOperation]->GetName() + "-" +
                      (Operation[currentOperation]->GetSnapshotsListItem(0)) +
                      ".log";
    ...
    } else // this is just to create the .snapDefaultDir if it does not to exist
           // so as to copy profile data later...
    {
      // we will create the snapshots default directory by using an rsync
      // command with an empty source without --delete option
      QProcess *mkdirProcess;
      mkdirProcess = new QProcess(this);
      QStringList mkdirArgs;
      mkdirArgs.clear();
      // no needed any more
      // if ( (WINrunning) && (RemoteDestUsed) )
      //     mkdirArgs << "--mkdir";
      ...
      if (RemoteDestUsed)
        // mkdirArgs.append(remoteArgs);   // use operator << instead of append
        // to maintain compatiiblity with debian 5
        mkdirArgs << remoteArgs;

      mkdirArgs.append(snapEmptyDir);

      // rsync throws error if directory is not yet created, ..so create it
      // first

      // Add the destination folder
      mkdirArgs.append(
          tempDestinationOrig); // this is actually an empty argument
      if (WINrunning) {
        // bool createWinRsyncCommand(tempDirPath,QFile command1,QFile
        // command2,bool vss,QString rsyncArgs,QString source,QString dest);
      ...
      mkdirArgs.append(
          tempDestinationOrig2); // this is actually an empty argument
      if (WINrunning) {
        // bool createWinRsyncCommand(tempDirPath,QFile command1,QFile
        // command2,bool vss,QString rsyncArgs,QString source,QString dest);
        QString command2 = createWinRsyncCommand(tempDirPath, false, mkdirArgs);
    ...
    Operation[currentOperation]->SetLastExecutionTime(
        QDateTime::currentDateTime());

    // add a new snapshot with the last execution date-time
    Operation[currentOperation]->AddSnapshotsListItem(
        (Operation[currentOperation]->GetLastExecutionTime())
            .toString("yyyyMMddhhmmss"));
    currentSnaps =
        Operation[currentOperation]
            ->GetSnapshotsListSize(); // update the current number of snapshots

    // set a new changes file. This has a tag of the previous snapshot will
    // include changes made to this snapshot
    snapchangesfilename =
        snapChangesDir + profileName + "-" +
        Operation[currentOperation]->GetName() + "-" +
        (Operation[currentOperation]->GetSnapshotsListItem(currentSnaps - 1)) +
        ".changes.log";
    snapfile.setFileName(snapchangesfilename);

    // set a new logfile
    logfilename =
        logDir + profileName + "-" + Operation[currentOperation]->GetName() +
        "-" +
        (Operation[currentOperation]->GetSnapshotsListItem(currentSnaps - 1)) +
        ".log";
    logfile.setFileName(logfilename); // this is the logfile
    if (logfile.open(QIODevice::WriteOnly |
                     QIODevice::Text)) // create a new log file
  ...
  // the previous pre-task command exited with an error(all repeatOnFail runs)
  // and the stop checkbox is checked
  if ((Operation[currentOperation]->GetExecuteBeforeListSize() ==
       currentBefore) ||
      (StopTaskExecution)) {
    currentBefore = 0;
    ExecuteBefore = false;
    ...
    StopTaskExecution = false;
    ProcReportedError =
        false; // This might change as soon as syncprocess will start ()
    outputInsert = logFileUpdate("pre-starting", "", currentBefore);

    ui.rsyncOutput->append(outputInsert);

    syncProcess->start(
        Operation[currentOperation]->GetExecuteBeforeListItem(currentBefore));
    syncProcess->waitForStarted(5000);

    // The reason for the below jump is that when a process reports an error it
    // does not emit finished() signals neither std output/errors
...
void luckyBackupWindow::executeAfterTask() {
  ...
  // -----------------------------------------------------------------------------------------------------
  // if there are no (more) post-task commands to be executed or we are here
  // because a pre/post-task command exited with an error or destination could
  // not be created
  if ((Operation[currentOperation]->GetExecuteAfterListSize() ==
       currentAfter) ||
      (DestCreateFail) || (StopTaskExecution)) {
    if (!DryRun) {
      logfile.close(); // close the logfile first (will create a new one for the
                       // next task)
      Operation[currentOperation]->SetLastExecutionErrors(
          errorCount); // update the last execution errors
    }
    ...
    currentOperation++;
    // increase currentOperation until next operation to be executed or end of
    // operations
    while ((currentOperation < TotalOperations) &&
           (!Operation[currentOperation]->GetPerform()))
      currentOperation++;
    ...
    syncProcess->start(
        Operation[currentOperation]->GetExecuteAfterListItem(currentAfter));
    syncProcess->waitForStarted(5000);
...
void luckyBackupWindow::executeRsync() {
  ...
  if (StopTaskExecution) // if a pre-task command exited with an error do not do
                         // anything
  {
    procFinished();
    return;
  ...
  sync = Operation[currentOperation]
             ->GetTypeSync(); // set sync variable according to checkstate of
                              // radiobutton operation type
  rsyncArguments =
      AppendArguments(Operation[currentOperation]); // set rsync arguments

  bool RemoteDestUsed =
      (Operation[currentOperation]->GetRemoteDestination()) &&
      (Operation[currentOperation]->GetRemote()); // Is remote dest used ?
  ...
  DestCreateFail = false; // This will become true if destination does not exist
  ...
  if (!sync) {
    ...
    if ((!destCreate.exists()) && (!RemoteDestUsed)) // local use ONLY
    {
      if (destCreate.mkpath(dirB))
        outputInsert.append(logFileUpdate(
            "rsync-standard",
            "<br>" + tr("Successfully created destination directory"), 0));
      else {
        outputInsert.append(logFileUpdate(
            "rsync-error",
            "<br>" + tr("Failed to create destination directory"), 0));
        ui.rsyncOutput->append(outputInsert);
        DestCreateFail = true;
        errorsFound++;
        errorCount++;
  ...
  if (WINrunning) {
    ...
    QString command2 = createWinRsyncCommand(
        tempDirPath, Operation[currentOperation]->GetOptionsVss(),
        rsyncArguments);
    ui.rsyncOutput->append("\n" + command2);
    if (command2 == "")
    ...
    syncProcess->start(
        command, rsyncArguments); // execute rsync command with rsyncArguments
...
// when rsyncProcess emits finished signal execute another RsyncProcess if any
// left====================================================================
void luckyBackupWindow::procFinished() {
  if (ABORTpressed) // this is to prevent segmentation fault when abort button
                    // pressed
    return;
  ...
  if (ExecuteBefore) // if the pre-task execution command (process) finished
  {
    ...
    // set this for a successful run of a command. They might change if there
    // was an error
    ExecuteBeforeExitedError = false;
    StopTaskExecution = false;
    ...
    if ((syncProcess->exitCode() != 0) || (ProcReportedError)) {
      ...
      if (repeatOnFailTry ==
          repeatOnFailMax) // if the last run of the command just ended
      {
        if (Operation[currentOperation]->GetExecuteBeforeListItemState(
                currentBefore) == true)
        ...
        ui.rsyncOutput->append(outputInsert);
        repeatOnFailTry++; // re-run this command, do not proceed to the next
                           // one
      }
  ...
  if (ExecuteAfter) // if the post-task execution command (process) finished
  {
    ...
    if ((syncProcess->exitCode() != 0) || (ProcReportedError)) {
      if (repeatOnFailTry ==
          repeatOnFailMax) // if the last run of the command just ended
      {
        if (Operation[currentOperation]->GetExecuteAfterListItemState(
                currentAfter) == true)
          StopTaskExecution = true;
  ...
  if (StopTaskExecution)
      ...
      if (snapfile.open(QIODevice::ReadOnly | QIODevice::Text)) {
        ...
        while (!in.atEnd()) {
          ...
          if (snapLine.contains("+++++++", Qt::CaseInsensitive)) {
            snapLine =
                snapLine.right(snapLine.size() - snapLine.lastIndexOf("[LB]") -
                               removeCharLimit) +
                "\n";
        ...
        snapfile.close();
        filesAdded.remove("[LB]",
                          Qt::CaseSensitive); // just to make sure cause
                                              // sometimes there are [LB]'s left
      }
    ...
    if ((!sync) || ((sync) && (syncAB))) {
      ...
      QString exportProfileDir = "";
      QString sourceLast = Operation[currentOperation]->GetSource();
      // calculate the last folder of source
      if (!sourceLast.endsWith(
              SLASH)) // this means task is of type "backup dir by name"
        sourceLast = calculateLastPath(
            sourceLast); // This is the lowest dir of the source
      else
        sourceLast = "";

      if (!rsyncArguments
               .isEmpty()) // rsyncArguments is calculated at executeRsync()
      {
        if (WINrunning && RemoteDestUsed)
          exportProfileDir = rsyncArguments.last() + sourceLast + XnixSLASH +
                             snapDefaultDir + profileName + ".profile" +
                             XnixSLASH;
        else
          exportProfileDir = rsyncArguments.last() + sourceLast + SLASH +
                             snapDefaultDir + profileName + ".profile" + SLASH;
      }
      ...
      // If this is a backup task && not a dryrun, backup profile data to
      // destination
      if ((!sync) && (!DryRun) && (!rsyncArguments.isEmpty())) {
        ...
        if (exportFullProfile(exportProfileDir, "ExportOnlyTask"))
          outputInsert =
              logFileUpdate("backup-profile", " -> Ok", currentAfter);
        else
          outputInsert =
              logFileUpdate("backup-profile", " -> Fail", currentAfter);
      }
  ...
  /* If there is an error repeat the rsync command. Errors:
                              ...
                              23 - Partial transfer due to error -> I took this
     out 30 - Timeout in data send/receive 35 - Timeout waiting for daemon
     connection 255 - unexplained error */
  if ((repeatOnFailTry < repeatOnFailMax) &&
      ((syncProcess->exitCode() == 30) || (syncProcess->exitCode() == 35) ||
       (syncProcess->exitCode() == 255) || (syncProcess->exitCode() == 12) ||
       (syncProcess->exitCode() == 5) || (ProcReportedError))) {
  ...
  if ((sync) &&
      (!syncAB)) // sync A->B is finished. Do the opposite now before proceeding
                 // to next included operation or post-task commands
    executeRsync();
  else
...
void luckyBackupWindow::procError() {
  ...
  case QProcess::FailedToStart:
    errorText = tr("Failed to start", "this refers to a process");
    break;
  case QProcess::Crashed:
    errorText = tr("Crashed", "this refers to a process");
    break;
  case QProcess::Timedout:
    errorText = tr("Timed out", "this refers to a process");
    break;
  case QProcess::WriteError:
    errorText = tr("Write error", "this refers to a process");
    break;
  case QProcess::ReadError:
    errorText = tr("Read error", "this refers to a process");
    break;
  case QProcess::UnknownError:
    errorText = tr("Unknown error", "this refers to a process");
    break;
  }
...
void luckyBackupWindow::appendRsyncOutput() {
  ...
  // progressbar--------------------------------------------------------------------------------------------------------
  bool ok;
  if ((outputString.contains("to-check")) ||
      (outputString.contains("to-chk"))) // we will calculate how many files
                                         // have been proccessed so far
  {
    ...
    if (outputString.contains("to-check")) // if rsync uses "to-check="
      DoneToTotal_Ref = outputString.midRef(
          outputString.indexOf("check=") + 6,
          outputString.indexOf(")") - outputString.indexOf("check=") - 6);
    else // if rsync uses "to-chk="
      DoneToTotal_Ref = outputString.midRef(
          outputString.indexOf("o-chk=") + 6,
          outputString.indexOf(")") - outputString.indexOf("o-chk=") - 6);
    QString DoneToTotal_String = DoneToTotal_Ref.toString();

    // Total no files
    QStringRef ref_temp = DoneToTotal_String.rightRef(
        DoneToTotal_String.size() - DoneToTotal_String.indexOf(SLASH) - 1);
    QString string_temp = ref_temp.toString();
    progress_total = string_temp.toInt(&ok, 10);
    ui.OperationProgress->setRange(
        0, progress_total); // set the range of the progressbar to the no of
                            // files to consider
    ...
    ui.OperationProgress->setValue(
        progress_done); // set the current progressbar value
  }
  if (outputString.contains("speedup is")) // the process has finished, so if
                                           // we're back fill it to 100%
  {
    ui.OperationProgress->setRange(0, 100);
  ...
  if ((outputString.contains("Number of files transferred")) ||
      (outputString.contains("Total bytes sent:")) ||
      (outputString.contains("Number of regular files"))) {
    ...
    foreach (QString line, lines) {
      if (line.contains("Number of files transferred:"))
        FilesTransferedString =
            line.remove("Number of files transferred: "); // old rsync report;
      if (line.contains("Number of regular files transferred:"))
        FilesTransferedString = line.remove(
            "Number of regular files transferred: "); // newer rsync report
      if (line.contains("Total transferred file size:"))
        BytesTransferedString =
            line.remove("Total transferred file size: "); // newer rsync report
      if (line.contains("Total bytes sent:"))
        BytesTransferedString =
            line.remove("Total bytes sent: "); // old rsync report
    }
    ...
    bytesTransfered =
        bytesTransfered + BytesTransferedString.toULongLong(&ok, 10);
  }
}
...
progressbar--------------------------------------------------------------------------------------------------------
            bool ok;
            if (outputString.contains("to-check"))  //we will calculate how many
files have been proccessed so far
            {
                //DoneToTotal_Ref & DoneToTotal_String hold a e.g. "17/84"
                QStringRef DoneToTotal_Ref =
outputString.midRef(outputString.indexOf("check=")+6,outputString.indexOf(")")-outputString.indexOf("check=")-6);
                QString DoneToTotal_String = DoneToTotal_Ref.toString();

                //Total no files
                QStringRef ref_temp =
DoneToTotal_String.rightRef(DoneToTotal_String.size() -
DoneToTotal_String.indexOf(SLASH) -1); QString string_temp =
ref_temp.toString(); progress_total = string_temp.toInt(&ok,10);
                ui.OperationProgress -> setRange(0,progress_total); //set the
range of the progressbar to the no of files to consider

                //No of files processed so far
                ref_temp =
DoneToTotal_String.leftRef(DoneToTotal_String.indexOf(SLASH)); string_temp =
ref_temp.toString(); progress_done = string_temp.toInt(&ok,10); progress_done =
progress_total - progress_done; ui.OperationProgress -> setValue
(progress_done);   //set the current progressbar value
            }
            if (outputString.contains("speedup is"))    //the process has
finished, so if we're back fill it to 100%
            {
                ui.OperationProgress -> setRange(0,100);
                ui.OperationProgress -> setValue (100);
            }
    ...
    if (pipeVssErrFile->open(QIODevice::ReadOnly | QIODevice::Text))
    {
        ...
        if (outputError !="")
        {
            errorsFound++;
            errorCount++;
            ui.rsyncOutput->append(logFileUpdate("rsync-error", outputError,
0));
        }
    }
...
void luckyBackupWindow::setNowDoing() {
  ...
  if ((currentOperation < TotalOperations) && (!ABORTpressed)) {
    if (ExecuteBefore)
      nowDoingText =
          "<p align=\"center\">" + tr("Elapsed time") +
          " : <b><font color=red>" + DifTime.toString("hh:mm:ss") +
          "</font></b><br>" + tr("pre-task execution of command") +
          "	: <b>" +
          Operation[currentOperation]->GetExecuteBeforeListItem(currentBefore) +
          "</b>";
    ...
    if (ExecuteAfter)
      nowDoingText =
          "<p align=\"center\">" + tr("Elapsed time") +
          " : <b><font color=red>" + DifTime.toString("hh:mm:ss") +
          "</font></b><br>" + tr("post-task execution of command") +
          "	: <b>" +
          Operation[currentOperation]->GetExecuteAfterListItem(currentAfter) +
          "</b>";

    if ((sync) && (!ExecuteAfter) &&
        (!ExecuteBefore)) // if a sync operation is executed
    {
      ...
      if (calculating)
        nowDoingText.append(
            tr("calculating", "info message displayed during ...calculations") +
            ": " + outputString);
      if (transferring)
        nowDoingText.append(
            tr("transferring files",
               "info message displayed during ...file transfers") +
            ": " + outputString);
      if (deleting)
        nowDoingText.append(
            tr("deleting files",
               "info message displayed during ...file deletions") +
            ": " + outputString);
    }

    if ((!sync) && (!ExecuteAfter) &&
        (!ExecuteBefore)) // if a backup operation is executed
    {
      nowDoingText = "<p align=\"center\">" + tr("Elapsed time") +
                     " : <b><font color=red>" + DifTime.toString("hh:mm:ss") +
                     "</font></b><br>" + tr("Now performing task") +
      ...
      nowDoingText.append(tr("Source") + "	: <b><font color=blue>" + dirA +
                          "</font></b><br>" + tr("Destination") +
                          "	: <b><font color=blue>" + dirB +
                          "</font></b><br>");
  ...
  if ((currentOperation == TotalOperations) &&
      (!ABORTpressed)) // if all operations finished normally - not aborted
  {
    NOWexecuting = false; // this is mainly used if the window close button (or
                          // alt+F4) is pressed
    ...
    nowDoingText = "<p align=\"center\">" + tr("Elapsed time") +
                   " : <b><font color=red>" + DifTime.toString("hh:mm:ss") +
                   "</font></b><br>" + tr("Total files transferred") +
                   " : <b>" + countStr.setNum(filesTransfered) + "</b> (" +
                   convertBytes(QString::number(bytesTransfered), true) +
                   ")<br>"
                   "========================================="
                   "<b><br><font color=blue>" +
                   tr("All tasks completed") + " </font></b>";
    trayMessage = tr("All tasks completed");
    if (DryRun) {
      nowDoingText.append(" <b><font color=magenta>(" + tr("simulation mode") +
                          ")</b></font>");
      trayMessage.append(" (" + tr("simulation mode") + ")");
    }
    if (errorsFound == 0) {
      nowDoingText.append("<br><font color=green>" + tr("No errors found") +
                          "</font><br>");
      trayMessage.append("\n" + tr("No errors found"));
    } else {
      nowDoingText.append("<br><font color=green>" + tr("errors found") + ": " +
                          countStr.setNum(errorsFound) + "</font><br>");
      trayMessage.append("\n" + tr("errors found"));
    ...
    if (!DryRun)
      nowDoingText.append(tr("logfile(s) have been created under directory: ") +
                          logDir + "<br>");
    nowDoingText.append("=========================================</p>");
    ui.AbortButton->setVisible(false);
    ui.DoneButton->setVisible(true);
    ...
    if ((QSystemTrayIcon::isSystemTrayAvailable()) &&
      ...
      if (KDErunning) {
        ...
        QStringList dialogArgs;
        dialogArgs << "--title"
                   << appName + " - " + tr("execution of profile:") + " " +
                          profileName + " " + tr("finished")
                   << "--passivepopup" << trayMessage << "10";
        dialogProcess->start("kdialog", dialogArgs);
      } else
        LBtray->showMessage(appName + " - " + tr("execution of profile:") +
                                " " + profileName + " " + tr("finished"),
    ...
    if ((silentMode) &&
        (isMinimizedToTray == true)) // if --silent is given as argument and the
                                     // gui is not shown exit the app
    {
  ...
  if (ABORTpressed) // if operations were terminated by user
  {
    ...
    ui.DoneButton->setVisible(true);
    ui.pushButton_minimizeToTray->setVisible(false);
    ui.rsyncOutput->append("<br><font color=red><b>" + tr("ABORTED") +
                           " !!</b></font>");
...
void luckyBackupWindow::finishUp() {
  if (!DryRun) {
    ...
    if ((!ABORTpressed) && (!emailNever)) {
      bool send = true;
      if (((emailError) &&
           (errorsFound == 0)) // do not send if the condition "error" is
                               // checked and there are no errors
          || ((emailSchedule) &&
              (!silentMode))) // do not send if the condition "scheduled" is
                              // checked and profile is not run in silent mode
  ...
  // QMessageBox::information(this, "LB","QtextDocument title= **" +
  // ui.rsyncOutput->documentTitle() +"**");
}

// function
// shutDownSystem=====================================================================================================
// shutdown the system if the relevant button is pressed, when all tasks are
// finished
  ...
  if (KDErunning) {
    ...
    if ((QSystemTrayIcon::isSystemTrayAvailable()) &&
      ...
      LBtray->showMessage(appName + " - " + tr("WARNING"),
                          tr("The system will shutdown in 1 minute"),
...
void luckyBackupWindow::previousErrorJump() {
  ...
  if (errorCount == 0) // if the current error is the first within the logfile
                       // disable the previous button
    ui.pushButton_previousError->setEnabled(false);

  if (errorCount <
      errorsFound - 1) // if the current error is less than the last one within
                       // the logfile enable the next button
    ui.pushButton_nextError->setEnabled(true);
...
void luckyBackupWindow::nextErrorJump() {
  ...
  if (errorCount == errorsFound - 1) // If the current error is the last within
                                     // the logfile disable the next button
    ui.pushButton_nextError->setEnabled(false);

  if (errorCount > 0) // if the current error is greater than the first one
                      // within the logfile enable the previous button
...
// if bool=false then conversion string->bytes. If bool=true then conversion
// bytes->string
// =====================================================================================================
QString luckyBackupWindow::convertBytes(QString byteLine, bool toWhat) {
  ...
  if (toWhat) // convert bytes to string
  {
    ...
    if ((returnThis.contains(".")) &&
        ((returnThis.lastIndexOf(".") + 3) < returnThis.size()))
      returnThis.chop(returnThis.size() - returnThis.lastIndexOf(".") -
                      3); // leave only 3 decimal points
