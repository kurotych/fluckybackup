...
// Constructor=================================================================================================
// Launches a wizard
manageWizard::manageWizard(QString type, QString SOURCE, QString DEST,
                           int snapshot, QWidget *parent)
    : QDialog(parent) {
  // initialize variables
  currentSnap = snapshot; // this is the current snapshot number
  snapshotsNo = Operation[currentOperation]
                    ->GetSnapshotsListSize(); // this is the number of snapshots
  wizard_type = type;
  snapToRestore =
      snapshotsNo - 2; // This means the previous of the very last snapshot
  ...
  if (Operation[currentOperation]->GetTypeDirContents())
    ...
    sourceLast = calculateLastPath(source) +
                 SLASH; // This is the lowest dir of the source
  ...
  if (time == "")
    ...
    timeReadable = time.mid(0, 4) + "/" + time.mid(4, 2) + "/" +
                   time.mid(6, 2) + " - " + time.mid(8, 2) + ":" +
                   time.mid(10, 2) + ":" + time.mid(12, 2); // QString
  ...
  // delete backup;if this are more than one snapshots available, set dir to
  // delete as snapSpecificDir
  ...
  connect(uiW.checkBox_DeleteAfter, SIGNAL(stateChanged(int)), this,
          SLOT(deleteAfterChanged()));

  commandProcess =
      new QProcess(this); // create a new qprocess (for rsync) & connect signals
  connect(commandProcess, SIGNAL(readyReadStandardError()), this,
          SLOT(appendCommandOutput()));
  connect(commandProcess, SIGNAL(readyReadStandardOutput()), this,
          SLOT(appendCommandOutput()));
  connect(commandProcess, SIGNAL(finished(int, QProcess::ExitStatus)), this,
          SLOT(procFinished()));

  connect(uiW.button_previous, SIGNAL(clicked()), this,
          SLOT(prevError())); // connect previous pushButton SLOT
  connect(uiW.button_next, SIGNAL(clicked()), this,
          SLOT(nextError())); // connect next pushButton SLOT
  connect(uiW.pushButton_changeRestore, SIGNAL(clicked()), this,
          SLOT(changeRestorePath())); // connect change pushButton SLOT
}
// SLOTS-------------------------------------------------------------------------------------
...
void manageWizard::deleteAfterChanged() {
  DeleteAfter = uiW.checkBox_DeleteAfter
                    ->isChecked(); // set bool DeleteAfter according to checkbox
                                   // (only applies at RESTORE)
  QString labelText = "<b><font color=red>" + tr("WARNING") + "</font></b>: ";
  if (DeleteAfter)
    labelText.append(tr(
        "The restore directory will become identical to the backup snapshot"));
  else
    labelText.append(tr("Existing backup files will replace the corresponding "
                        "files at the restore directory",
                        "information message - line1."));

  labelText.append(
      ".<br>" +
      tr("If the information above is correct, click <b>start</b> to begin",
         "information message - line2.\nPlease leave tags <b></b> intact and "
         "surrounding 'start translated'") +
      ".");
...
void manageWizard::startAction() {
  DeleteAfter = uiW.checkBox_DeleteAfter
                    ->isChecked(); // set bool DeleteAfter according to checkbox
                                   // (only applies at RESTORE)
  DryRun =
      uiW.checkBox_DryRun->isChecked(); // set bool DeleteAfter according to
                                        // checkbox (only applies at RESTORE)
  ...
  // logfile actions if real run is performed - is always true for DELETE
  // actions
  // This is the RESTORE/DELETE logfile
  if (!DryRun) {
    if (wizard_type == "restoreBackup")
      logfilename = logDir + profileName + "-" +
                    Operation[currentOperation]->GetName() + "-" + time +
                    "-RESTORE" + ".log";
    else
      logfilename = logDir + profileName + "-" +
                    Operation[currentOperation]->GetName() + "-" + time +
...
void manageWizard::procFinished() {
  ...
  if ((wizard_type == "deleteBackup") && (errorsFound == 0) && (!procKilled)) {
    ...
    uiW.actionView->append(
        "<br><font color=blue><b>..." +
        tr("Deleting snapshot logfile",
           "info message displayed during ...logfile deletion") +
        "</b></font><br>");
    QString logNameToDelete = logDir + profileName + "-" +
                              Operation[currentOperation]->GetName() + "-" +
                              time + ".log";
    QFile logToDelete;
    logToDelete.setFileName(
        logNameToDelete); // this is the logfile of the snapshot to delete
    if (logToDelete.remove())
      uiW.actionView->append(
          "	<font color=green>" +
          tr("success", "info message displayed after ...logfile deletion") +
          "</font><br><br>");
    else
      uiW.actionView->append(
          "	<font color=red>" +
          tr("failed", "info message displayed after ...logfile deletion") +
          "</font><br><br>");

    // delete the .changes file
    uiW.actionView->append(
        "<font color=blue><b>..." +
        tr("Deleting file that lists snapshot changes",
           "info message displayed during ...file deletion") +
        "</b></font><br>");
    QString changesNameToDelete = snapChangesDir + profileName + "-" +
                                  Operation[currentOperation]->GetName() + "-" +
                                  time + ".changes.log";
    QFile changesFileToDelete;
    changesFileToDelete.setFileName(
        changesNameToDelete); // this is the .changes file of the snapshot to
                              // delete
    if (changesFileToDelete.remove())
      uiW.actionView->append(
          "	<font color=green>" +
          tr("success", "info message displayed after ...file deletion") +
          "</font><br><br>");
    else
      uiW.actionView->append(
          "	<font color=red>" +
          tr("failed", "info message displayed after ...file deletion") +
          "</font><br><br>");

    // remove the specific snapshot from the Task
    uiW.actionView->append(
        "<font color=blue><b>..." +
        tr("Updating list of snapshots",
           "info message displayed during ...snaps list update") +
        "</b></font><br>");
    Operation[currentOperation]->RemoveSnapshotsListItem(currentSnap);
    uiW.actionView->append(
        "	<font color=green>" +
        tr("success", "info message displayed after ...snapshots list update") +
        "</font><br><br>");
    ...
    // update the last execution time to the last snapshot or to nothing if no
    // snapshots exist
    QDateTime newTime; // empty time variable
    if (!emptySnapList)
      newTime = QDateTime::fromString(
  ...
  if (errorsFound == 0)
    InfoMessage.append("<font color=green><b>" + tr("no errors") +
                       "</b></font>");
  else
    InfoMessage.append("<font color=red><b>" + tr("errors found") +
                       "</b></font>");
...
void manageWizard::abortAction() {
  if (commandProcess->state() == QProcess::Running) {
    ...
    procFinished(); // the process might not be running for some weird reason,
                    // so call procFinished !!
}
// appendCommandOutput
// =====================================================================================================
...
void manageWizard::changeRestorePath() {
  QString newRestore = QFileDialog::getExistingDirectory(
      this,
      tr("Select new restore directory", "directory selection dialog title"),
...
void manageWizard::prevError() {
  ...
  if (errorCount ==
      0) // if the current error is the first disable the previous button
    uiW.button_previous->setEnabled(false);

  if (errorCount < errorsFound - 1) // if the current error is less than the
                                    // last one, enable the next button
    uiW.button_next->setEnabled(true);
...
void manageWizard::nextError() {
  ...
  if (errorCount ==
      errorsFound -
          1) // If the current error is the last, disable the next button
    uiW.button_next->setEnabled(false);
...
void manageWizard::guiInitialize() {
  ...
  if (wizard_type == "deleteBackup") {
    ...
    uiW.groupBox_title->setTitle(
        tr("Delete backup", "this is the title of a wizard"));
    uiW.label_time->setText(
        tr("Date & time", "simple label of the wizard gui") +
        ": <font color=magenta><b>" + timeReadable + "</b></font>");
    uiW.label_paths->setText(tr("Path", "...is a directory path") +
                             ": <font color=blue><b>" + dest + "</b></font>");
    uiW.label_message->setText(
        "<b><font color=red>" + tr("WARNING") + "</font></b>: " +
        tr("You are about to delete backup data",
           "information message - line1") +
        ".<br>" +
        tr("If the information above is correct, click <b>start</b> to begin",
           "information message - line2.\nPlease leave tags <b></b> intact and "
  ...
  if (wizard_type == "restoreBackup") {
    ...
    uiW.groupBox_title->setTitle(
        tr("Restore backup", "this is the title of a wizard"));
    uiW.label_time->setText(
        tr("Date & time", "simple label of the wizard gui") +
        ": <font color=magenta><b>" + timeReadable + "</b></font><br>" +
        tr("Backup path", "...is the backup directory path") +
        ": <font color=blue><b>" + dest + "</b></font>");

    uiW.label_paths->setText(
        tr("Restore path", "...is the restore directory path") +
...
void manageWizard::calcCommandArgs() {
  ...
  commandArguments.removeLast(); // remove the last 2 arguments (hopefuly source
                                 // & destination)
  ...
  while (count < commandArguments.size()) {
    if ((commandArguments[count].contains("--exclude")) ||
      ...
      commandArguments.removeAt(
          count); // remove the argument if one of the above is true
    else
      count++;
  }
  ...
  // this means read the changes.logs files from every snapshot after the
  // current one
  count = currentSnap + 1;
  while (count < snapshotsNo) {
    commandArguments.append(
        "--exclude-from=" + snapChangesDir + profileName + "-" +
        Operation[currentOperation]->GetName() + "-" +
  ...
  if (MainRun) {
    // exclude the luckybackup snapshots directory cause it is inside the
    // destination
    ...
    commandArguments.append(dest); // set new dest as source Argument  - as it
                                   // was calculated at manageWizard
  } else {
    QString tslash;
    if (WINrunning && Operation[currentOperation]->GetRemote())
      tslash = XnixSLASH;
  ...
  commandArguments.append(
      source); // set new source as destination ...as it was calculated at
               // manageWizard or changed by changeRestorePath
}
...
void manageWizard::runProcess() {
  ...
  QString commandLocal = "";
  commandArguments.clear(); // the local command is used here !!
  ...
  if (wizard_type == "restoreBackup") {
    ...
    if (MainRun)
      InfoMessage.append(
          tr("Restoring data: main trunk",
             "info message displayed during ...data restoration"));
    else {
      InfoMessage.append(
          tr("Restoring data: snapshot files",
             "info message displayed during ...data restoration") +
          " - " + time);
    ...
    InfoMessage.append(
        tr("Deleting data", "info message displayed during ...data deletion"));
  ...
  if (skipTHIS)
    InfoMessage.append(
        "<br><font color=blue>" +
        tr("No snapshot specific data exist. Skipping...",
           "info message displayed during ...data restoration/deletion") +
        "</font><br>");
  ...
  // Do not start specific snapshot data restoration if the relevant snapshots
  // dir does not exist or do not delete the snapshots folder if it does not
  // exist
  if (skipTHIS)
    procFinished();
  else {
