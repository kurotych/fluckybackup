...
commandline::commandline() {
  writeToLog = false;
  errorsFound = 0;     // Total number of errors from all tasks (in CLI, only 1
                       // error per task is allowed !!)
...
// result===================================================================================================================
// calls functions intro, loadProfile, check_list, check_dirs, rsyncIT,
// thats_all and returns an integer to main()
...
void commandline::intro() {
  ...
  cout << "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
          "~~~~~~~~~~~~~~~~~~~~~~~~\n\n";
  cout << "This application will (hopefully) sync and backup everything you "
          "need in no time\n";
  ...
  {
    cout << "Make sure that:\n";
    cout << "You have already declared and mounted all the directories that "
            "need to be synced/backed-up\n";
    cout << "Please take a moment to examine all messages from checks done "
            "until now\n\n";
    cout << "Also have in mind that, to avoid errors, it is best to run this "
            "app as su\n\n";
...
void commandline::rsyncIT() {
  ...
  syncProcess = new QProcess; // create a new qprocess (for rsync)
  QString command =
      rsyncCommandPath;  // command to be executed. Normally this is "rsync"
  QString command2 = ""; // windows use
  QStringList rsyncArguments; // This stringList holds all arguments for the
                              // rsync command
  ...
  while (currentOperation < TotalOperations) {
    if (WINrunning) {
      ...
      /* disable vss until...
      pipeVssFile =  new QFile(Operation[currentOperation] ->
      GetTempPath()+SLASH+"qt_tempvss"+QString::number(qrand() % (999998) + 1));
      if (pipeVssFile->open(QIODevice::ReadWrite)){
          pipeVssFile->close();
      }
      pipeVssErrFile =  new QFile(Operation[currentOperation] ->
      GetTempPath()+SLASH+"qt_tempvsserr"+QString::number(qrand() % (999998) +
      1)); if (pipeVssErrFile->open(QIODevice::ReadWrite)){
    ...
    if ((Operation[currentOperation]->GetPerform()) ||
        (Operation[currentOperation]->GetByPassWARNING())) {
      bool RemoteDestUsed =
      ...
      // get created so that the profile will be backed up without problems
      // later on First calculate the folder where snapshots go
      ...
      QString tempDestinationOrig =
          tempArguments[tempArguments.size() - 1]; // windows use
      ...
      if (!DryRun) {
        ...
        if (maxSnaps < 1)
          maxSnaps = 1;
        int currentSnaps = Operation[currentOperation]
                               ->GetSnapshotsListSize(); // this is the current
                                                         // number of snapshots
        if (currentSnaps < 1)
          currentSnaps = 1;
        ...
        // Juan's patch
        // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        // this is just to create the .snapDefaultDir if it does not to exist so
        // as to copy profile data later...
        ...
        if (WINrunning) {
          ...
          command2 =
              createWinRsyncCommand(Operation[currentOperation]->GetTempPath(),
                                    false, mkdirArgs, false);
          if (command2 == "")
        ...
        //                    QTemporaryFile
        //                    command1(QDir::tempPath()+"\\qt_tempXXXXXX.bat");
        //                    QTemporaryFile
        //                    command2(QDir::tempPath()+"\\qt_tempXXXXXX.bat");
        if (WINrunning) {
          // bool createWinRsyncCommand(tempDirPath,QFile command1,QFile
          // command2,bool vss,QString rsyncArgs,QString source,QString dest);
          command2 =
        ...
        // first remove the older logfiles and snapshots if max keep snapshots
        // is reached
        if (currentSnaps >= maxSnaps) {
          ...
          // increase the remove limit to include the source.size() if "backup
          // dir by name" is used
          ...
          if (WINrunning) {
            // bool createWinRsyncCommand(tempDirPath,QFile command1,QFile
            // command2,bool vss,QString rsyncArgs,QString source,QString dest);
            command2 = createWinRsyncCommand(
                Operation[currentOperation]->GetTempPath(), false, rmArgs,
                false);
            if (command2 == "")
              cout << "\nfailed to create bat file in rmProccess";
            else {
          ...
          while (count < (currentSnaps - maxSnaps + 1)) {
            ...
            snapchangesfilename =
                snapChangesDir + QString(profileName.toUtf8()) + "-" +
                QString((Operation[currentOperation]->GetName()).toUtf8()) +
            ...
            if (snapfile.exists()) {
              if (snapfile.remove()) // this is the old snapshot changes file
                cout << "\nRemoving "
                     << QString(snapchangesfilename.toUtf8()).toStdString();
              else
                cout << "\nfailed to remove "
                     << QString(snapchangesfilename.toUtf8()).toStdString()
                     << "\n";
            ...
            if (logfile.exists()) {
              if (logfile.remove())
                cout << "\nRemoving "
                     << QString(logfilename.toUtf8()).toStdString();
              else
                cout << "\nfailed to remove "
                     << QString(logfilename.toUtf8()).toStdString() << "\n";
            }
        ...
        // the following is transfered in the very beginning of the actions for
        // !DryRun,
        /*else        // this is just to create the .snapDefaultDir if it does
        not to exist so as to copy profile data later...
        {
            //we will create the snapshots default directory by using an rsync
        command with an empty source without --delete option QProcess
        *mkdirProcess;     mkdirProcess  = new QProcess; QStringList mkdirArgs;
        mkdirArgs.clear(); if ( (WINrunning) && (RemoteDestUsed) ) mkdirArgs <<
        "--mkdir"; else mkdirArgs << "--progress" << "-r";
        ...
        // set the current date and time as the operation's last execution
        // date-time
        ...
        currentSnaps = Operation[currentOperation]
                           ->GetSnapshotsListSize(); // update the current
                                                     // number of snapshots
        ...
        snapchangesfilename =
            snapChangesDir + QString(profileName.toUtf8()) + "-" +
            QString((Operation[currentOperation]->GetName()).toUtf8()) + "-" +
            (Operation[currentOperation]->GetSnapshotsListItem(currentSnaps -
                                                               1)) +
            ".changes.log";
        snapfile.setFileName(snapchangesfilename);
        ...
        if (logfile.open(QIODevice::WriteOnly | QIODevice::Text |
                         QIODevice::Append)) // create a new log file
        {
      ...
      // The following part will be run as many times as the RepeatOnFail
      // argument It includes execute before, task & execute after actions
      int repeatTaskOnFailMax =
          Operation[currentOperation]
              ->GetRepeatOnFail(); // This holds the number of re-runs to try if
                                   // the task fails for a reason
      ...
      while (
          (Operation[currentOperation]->GetExecuteBeforeListSize() > count) &&
          (!StopTaskExecution)) {
        cout << "\n~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
                "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n";
        cout << " Now executing pre-task command:  "
             << QString((Operation[currentOperation]->GetExecuteBeforeListItem(
                             count))
                            .toUtf8())
                    .toStdString()
             << "\n";
        ...
        while ((RunTry < repeatTaskOnFailMax + 1) && (runNOW)) {
          ...
          if (RunTry > 0) {
            cout << "\n Repeating execution of pre-task command due to failure "
                    "-  run "
                 << RunTry + 1 << " of " << repeatTaskOnFailMax + 1 << "\n\n";
            logFileUpdate("repeat-on-fail", "", count); // update the logfile
          }

          // syncProcess -> execute (Operation[currentOperation] ->
          // GetExecuteBeforeListItem(count));
          syncProcess->start(
              Operation[currentOperation]->GetExecuteBeforeListItem(count));

          // wait for the process to start and finish
          if (syncProcess->waitForStarted(
                  -1)) // the "-1" arguments are used to avoid timing out
          {
          ...
          RunTry++;
        } // Main repeat-On-Fail loop -> The specific command will be repeated
      ...
      if (StopTaskExecution) {
        ...
        cout << "\n~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
                "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n";
        cout << " Pre-task command exited with an error. Skipping task "
                "execution...";
        cout << "\n------------------------------------------------------------"
                "--------------------------------\n";
      } else {
        ...
        bool DestCreateFail = false; // This will become true if destination
        ...
        if ((!destCreate.exists()) &&
            (!Operation[currentOperation]->GetTypeSync()) &&
            (!RemoteDestUsed)) // local use ONLY
        {
          if (destCreate.mkpath(dirB.toUtf8())) {
            logFileUpdate("rsync-standard",
                          "<br>Successfully created destination directory", 0);
            cout << "\n\n Successfully created destination directory\n";
          } else {
            logFileUpdate("rsync-error",
                          "<br>Failed to create destination directory", 0);
        ...
        if (!DestCreateFail) {
          ...
          // syncProcess -> execute (command,rsyncArguments);	// execute rsync
          // command with rsyncArguments for backup or sync A->B

          // start the repeat on fail loop for the rsync command (backup or
          // sync1)
          RunTry = 0;    // This run of the command is No...
          runNOW = true; // run it this time
          while ((RunTry < repeatTaskOnFailMax + 1) && (runNOW)) {
            runNOW = false; // do NOT run this another time if it goes ok
            if (RunTry > 0) {
              cout << "\n Repeating execution of rsync command due to failure "
                      "-  run "
                   << RunTry + 1 << " of " << repeatTaskOnFailMax + 1 << "\n\n";
              logFileUpdate("repeat-on-fail", "", count); // update the logfile
            }
            ...
            if (WINrunning) {
              // bool createWinRsyncCommand(tempDirPath,QFile command1,QFile
              // command2,bool vss,QString rsyncArgs,QString source,QString
              // dest);
              command2 = createWinRsyncCommand(
                  Operation[currentOperation]->GetTempPath(),
                  Operation[currentOperation]->GetOptionsVss(), rsyncArguments,
                  false);
              if (command2 == "")
            ...
            if ((syncProcess->exitCode() == 30) ||
                (syncProcess->exitCode() == 35) ||
                (syncProcess->exitCode() == 255) ||
                (syncProcess->exitCode() == 12) ||
                (syncProcess->exitCode() == 5))
              runNOW = true;
          ...
          if (Operation[currentOperation]->GetTypeSync())
            ...
            if (snapfile.open(QIODevice::ReadOnly | QIODevice::Text)) {
              ...
              while (!in.atEnd()) {
                ...
                if (snapLine.contains("+++++++", Qt::CaseInsensitive)) {
                  snapLine = snapLine.right(snapLine.size() -
                                            snapLine.lastIndexOf("[LB]") -
                                            removeCharLimit) +
                             "\n";
            ...
            if (!rsyncArguments.isEmpty()) // rsyncArguments is calculated at
                                           // executeRsync()
            {
              if (WINrunning && RemoteDestUsed)
                exportProfileDir = rsyncArguments.last() + sourceLast +
                                   XnixSLASH + snapDefaultDir + profileName +
                                   ".profile" + XnixSLASH;
              else
            ...
            if ((!Operation[currentOperation]->GetTypeSync()) && (!DryRun) &&
                (!rsyncArguments.isEmpty())) {
              // Create the export path if it does not exist
              QDir exportPathCreate(exportProfileDir);
              if ((!exportPathCreate.exists()) &&
                  (!RemoteDestUsed)) // local use ONLY
          ...
          if (Operation[currentOperation]
                  ->GetTypeSync()) // execute rsync for syncing B -> A
          {
            logFileUpdate("rsync-starting-syncBA", "", 0);
            cout << "\n\n Now executing 2nd part of sync task...\n";

            dirA = rsyncArguments[rsyncArguments.size() -
                                  2]; // swap last 2 arguments (dir names)
            dirB = rsyncArguments[rsyncArguments.size() - 1];
            ...
            while ((RunTry < repeatTaskOnFailMax + 1) && (runNOW)) {
              runNOW = false; // do NOT run this another time if it goes ok
              if (RunTry > 0) {
                cout << "\n Repeating execution of rsync command due to "
                        "failure -  run "
                     << RunTry + 1 << " of " << repeatTaskOnFailMax + 1
                     << "\n\n";
                logFileUpdate("repeat-on-fail", "", count); // update the
                                                            // logfile
              }

              // syncProcess -> execute (command,rsyncArguments);	//
              // execute rsync command with rsyncArguments B->A
              if (WINrunning) {
                // bool createWinRsyncCommand(tempDirPath,QFile command1,QFile
                // command2,bool vss,QString rsyncArgs,QString source,QString
                // dest);
                command2 = createWinRsyncCommand(
                    Operation[currentOperation]->GetTempPath(),
                    Operation[currentOperation]->GetOptionsVss(),
                    rsyncArguments, false);
                if (command2 == "")
                  cout << "\nfailed to create bat file in rmProccess";
                else
                  syncProcess->start(command2);
          ...
          count = 0;
          while ((Operation[currentOperation]->GetExecuteAfterListSize() >
                  count) &&
                 (!StopTaskExecution)) {
            cout << "\n~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
                    "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n";
            cout << " Now executing post-task command:  "
                 << QString(Operation[currentOperation]
                                ->GetExecuteAfterListItem(count)
                                .toUtf8())
                        .toStdString()
                 << "\n";
            ...
            while ((RunTry < repeatTaskOnFailMax + 1) && (runNOW)) {
              runNOW = false; // do NOT run this another time if it goes ok
              if (RunTry > 0) {
                cout << "\n Repeating execution of post-rsync command due to "
                        "failure -  run "
                     << RunTry + 1 << " of " << repeatTaskOnFailMax + 1
                     << "\n\n";
                logFileUpdate("repeat-on-fail", "", count); // update the
          ...
          if (StopTaskExecution) {
            ...
            cout << "\n~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
                    "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n";
            cout << " Post-task command exited with an error. Skipping "
                    "executing the rest of commands...";
            cout << "\n--------------------------------------------------------"
                    "------------------------------------\n";
          }
        }
...
void commandline::thats_all() {
  ...
  if (errorsFound == 0)
    ...
    cout << "\n			errors found: "
         << countStr.setNum(errorsFound).toStdString();
  if (writeToLog)
    cout << "\n 	logfile(s) have been created under directory: "
         << logDir.toStdString();
  ...
  if ((!emailNever) && (!DryRun)) {
    bool send = true;
    if (((emailError) &&
         (errorsFound == 0)) // do not send if the condition "error" is checked
                             // and there are no errors
        ||
        ((emailSchedule) &&
         (!NoQuestions))) // do not send if the condition "scheduled" is checked
