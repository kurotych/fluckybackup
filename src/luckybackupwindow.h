...
public:
  ...
  // general variables
  bool taskClicked; // these 2 bool vars help to determine when a checkbox state
                    // is changed for SLOT taskStateChanged
  bool taskChanged;
  bool savedProfile; // Becomes true if a profile is saved and false if it is
                     // modified

  bool GoBack; // Becomes true if the user tries to load a profile, but regrets
               // it !!
  ...
  int InfoInt; // this helps to define which message will appear at the info
               // window
  int count;   // simple count variable !!

private slots:
  void profileComboChanged();      // SLOT to set the currentProfile when
                                   // profileCombo index changes
  ...
  void checkCurrentItem(
      QListWidgetItem
          *); // SLOT: Check the currently selected operation and set an icon
  void taskStateChanged(); // SLOT: Sets the profile state to unsaved if a task
                           // checkbox is changed
  void refreshList(); // SLOT: Refresh the operations list when refresh button
                      // pressed
  void start(); // SLOT: Open start operations dialog when Button clicked()
  void
  modify(); // SLOT: Modify an existing task. Will also save it if bool is true
  ...
  void procFinished(); // SLOT executed when syncProcess is finshed (will start
                       // a new syncProcess if any left)
  void
  procError(); // SLOT for when a process reports an error (eg: failed to start)
...
private:
  ...
  int isProfileSaved(); // function to check if the current profile is saved and
                        // proceed or not
  void
  checkDeclared(); // function to check all declared data of included operations
  bool checkOperationList(); // function to first check the operations list for
                             // errors (eg nothing is included)
  void retranslateUi(); // function to retranslate the ui when user changes the
                        // gui language
  void createMenus(); // create the menu "language" with all available languages
                      // & menu "help"
  void createToolbar();      // create the toolbar
  void createActions();      // create all actions
  void createProfileCombo(); // fill the profile combo box with existing profile
                             // names
  bool saveSettings(); // saves various luckybackup settings such as the default
                       // profile
  ...
  bool arrangeLogSnap(bool, QString,
                      QString); // Rename-delete-copy logs & snaps when a
                                // profile/task is renamed, deleted,duplicated
  ...
  QString manualChapter;   // holds the specific chapter of the manual. changes
                           // depending on which window is open
  ...
  bool IsVisibleProfileComboToolbar; // holds the visible state of the profile
                                     // combo toolbar
  bool IsVisibleProfileToolbar;      // holds the visible state of the profile
                                     // actions toolbar
  bool IsVisibleProfileStartToolbar; // holds the visible state of the profile
                                     // start toolbar
  ...
  bool showOnlyErrors; // This becomes true if the user selects to only display
                       // errors at the commands output window
  ...
  void finishUp(); // finish up some stuff when all tasks finish either normally
                   // or aborted
  void
  shutDownSystem(); // shutdown the system if the relevant button is pressed
  QString convertBytes(
      QString,
      bool); // Converts a string of the form of 10GB to bytes and vice versa
  ...
  QString outputInsert; // this is used to enter application output between
                        // outputString/Error
  ...
  bool calculating; // these 3 bools are used to diplay progress of rsync at the
                    // info window
  bool transferring;
  bool deleting;
