/* ---------------------------------- functions.cpp
--------------------------------------------------------------------------- file
containing all functions for luckybackupwindow
...
void luckyBackupWindow::InitializeVariables() {
  ...
  // It is commented because the super use cannot use eg the tray notification
  // anyway, so he/she will use the qt stuff
  /*if (currentUser == "super user")
  {
      ...
      if (kdesuProcess -> exitCode() == 0)        // there is a root process
  "kdesu_stub running KDErunning = true;
  }*/
  ...
  if (manual.exists()) // if the manual is located in
                       // /usr/share/doc/packages/luckybackup (normal
                       // installation) - openSuse
    helpURL.setUrl(suseManual);
  manual.setFileName(systemManual);
  if (manual.exists()) // if the manual is located in /usr/share/doc/luckybackup
                       // (normal installation)
    helpURL.setUrl(systemManual);
  manual.setFileName(relativeManual);
  if (manual.exists()) // if the manual is located in relative path manual/ (app
  ...
  if (license.exists()) // if the licence file is located in the normal debian
                        // directory
    licenseURL.setUrl(debianLicense);
  license.setFileName(suseLicense);
  if (license.exists()) // if the licence file is located in the normal suse
                        // directory
    licenseURL.setUrl(suseLicense);
  license.setFileName(systemLicense);
  if (license.exists()) // if the licence file is located in the normal
                        // installation directory
    licenseURL.setUrl(systemLicense);
  license.setFileName(relativeLicense);
  if (license.exists()) // if the licence file is located in the same directory
                        // (compile directory)
...
// retranslateUi
// ===================================================================================================================================
// retranslate the ui every time the user chooses a different language from the
// menu
void luckyBackupWindow::retranslateUi() {
  ui.menuFile->setTitle(tr("&Profile", "This is a top menu item"));
  ui.actionRefresh->setText(
      tr("&Refresh", "This is a top 'Profile' menu action"));
  ui.actionDefault->setText(
      tr("De&fault", "This is a top 'Profile' menu action"));
  ui.actionDefault->setToolTip(
      tr("Set as Default", "This is a top 'Profile' menu action tooltip"));
  ui.actionDescription->setText(
      tr("&View/Edit Description", "This is a top 'Profile' menu action"));
  ui.actionDescription->setToolTip(
      tr("View/Edit the profile description",
         "This is a top 'Profile' menu action tooltip"));
  ui.actionRename->setText(
      tr("R&ename", "This is a top 'Profile' menu action"));
  ui.actionDelete->setText(
      tr("&Delete", "This is a top 'Profile' menu action"));
  ...
  ui.actionExport->setText(
      tr("E&xport", "This is a top 'Profile' menu action"));
  ui.actionImport->setText(
      tr("&Import", "This is a top 'Profile' menu action"));
  ui.actionSchedule->setText(
      tr("S&chedule", "This is a top 'Profile' menu action"));
  ui.actionEmail->setText(tr("E&mail", "This is a top 'Profile' menu action"));
  ui.actionEmail->setToolTip(tr("Email report after profile execution",
                                "This is a top 'Profile' menu action tooltip"));
  ui.actionQuit->setText(tr("&Quit", "This is a top 'Profile' menu action"));
  ui.actionDuplicate_Profile->setText(
      tr("D&uplicate", "This is a top 'Profile' menu action"));

  ui.menu_Task->setTitle(tr("&Task", "This is a top menu action"));
  ui.action_TaskAdd->setText(tr("&Add", "This is a top 'Task' menu action"));
  ui.action_TaskRemove->setText(
      tr("&Remove", "This is a top 'Task' menu action"));
  ui.action_TaskModify->setText(
      tr("&Modify", "This is a top 'Task' menu action"));
  ui.action_TaskDuplicate->setText(
      tr("Create D&uplicate task", "This is a top 'Task' menu action"));
  ui.action_TaskCreateRestore->setText(
      tr("Create R&estore task", "This is a top 'Task' menu action"));
  ...
  visibleProfileComboToolbar->setText(
      tr("Current Profile", "This is a top menu action"));
  actionLockToolbars->setText(
      tr("Lock", "This is a top menu action. Refers to toolbars"));
  actionVisibleToolbarText->setText(
      tr("Show text under icons", "This is a top menu action"));
  ...
  if (WINrunning) {
    ...
    actionSetWinPaths->setToolTip(tr("Set paths for rsync and ssh commands",
                                     "This is a top menu action tooltip"));
  }

  helpMenu->setTitle(tr("&Help", "This is a top menu item"));
  actionHelp->setText(appName + " " +
                      tr("&Handbook", "full phrase: 'luckyBackup Handbook'"));
  actionAbout->setText(tr("&About", "full phrase: 'about luckyBackup'") + " " +
                       appName);
  ...
  ui.pushButton_up->setToolTip(
      tr("Move the highlighted task up, by one position", "button tooltip"));
  ui.pushButton_down->setToolTip(
      tr("Move the highlighted task down, by one position", "button tooltip"));
  ui.pushButton_nextError->setToolTip(
      tr("jump to next error", "button tooltip"));
  ui.pushButton_previousError->setToolTip(
      tr("jump to previous error", "button tooltip"));
  ui.checkBox_onlyShowErrors->setText(tr("quiet mode"));
  ui.checkBox_onlyShowErrors->setToolTip(
      tr("Only show errors and important messages during window update"));

  ui.groupBox_task->setTitle(
      tr("Task", "task groupbox (add, remove, modify buttons) label"));
  ...
  ui.pushButton_remove->setToolTip(
      tr("remove highlighted task", "remove task button tooltip"));
  ui.pushButton_edit->setText(tr("modify", "modify task button label"));
  ui.pushButton_edit->setToolTip(
      tr("modify highlighted task", "modify task button tooltip"));

  // ui.groupBox_operations -> setTitle(tr("Run","run groupbox label"));
  ui.pushButton_start->setText(tr("Run", "start button label"));
  ui.pushButton_start->setToolTip(
      tr("Begin the execution of all included tasks", "start button tooltip"));
  ui.AbortButton->setText(tr("Abort", "Abort button label"));
  ui.AbortButton->setToolTip(
      tr("Stop the execution of running tasks NOW", "Abort button tooltip"));
  ui.DoneButton->setText(tr("Done"));
  ui.DoneButton->setToolTip(
      tr("Execution of tasks finished", "Done button tooltip"));
  ui.checkBox_DryRun->setText(
      tr("Dry", "simulation checkbox label. Translate this as 'simulation'"));
  ui.checkBox_DryRun->setToolTip(
      tr("This will perform a <b>simulation run</b> that doesn't make any "
         "changes (and produces mostly the same output as a real run)",
         "simulation checkbox tooltip - line1. Please leave tags <b> and </b> "
         "intact and surrounding 'simulation run'") +
      ".<br>" + tr("NOTE", "simulation checkbox tooltip - line2") + ": " +
      tr("Progressbar update will not be realistic",
         "simulation checkbox tooltip - line2"));

  ui.pushButton_shutdown->setText(tr("shutdown", "shutdown button label"));
  ui.pushButton_shutdown->setToolTip(
      tr("Press down to shutdown the system when done",
         "shutdown button tooltip"));

  ui.label_InformationWindow->setText(
      tr("Information window", "information window title"));
  ui.pushButton_exit->setToolTip(
      tr("Exit", "exit button tooltip. full phrase is: 'exit luckybackup'") +
      " " + appName);
  ...
  if (IsVisibleInfoWindow)
    ui.pushButton_InfoCollapse->setToolTip(
        tr("Hide information window",
           "show/hide information window button tooltip"));
  else
    ui.pushButton_InfoCollapse->setToolTip(
  ...
  ui.pushButton_minimizeToTray->setToolTip(
      tr("minimizes the window to the tray area"));
}
...
// Executes when the app starts & when the list of profiles in
// ~/.luckybackup/profiles changes
void luckyBackupWindow::createProfileCombo() {
  GoBack = true; // the next line will execute function setCurrentProfile. set
                 // true to avoid that
  ...
  for (count = 0; count < profileNames.size(); ++count) {
    ...
    if (currentProfile ==
        profileDir + profilename) // set the currentProfileIndex to the index of
                                  // the current profile
...
void luckyBackupWindow::createActions() {
  ...
  if (WINrunning) {
    ...
    connect(actionSetWinPaths, SIGNAL(triggered()), this,
            SLOT(setWinPaths())); // menu action set paths
  }

  connect(actionHelp, SIGNAL(triggered()), this,
          SLOT(help())); // menu action help
  connect(actionAbout, SIGNAL(triggered()), this,
          SLOT(about())); // menu action about
  connect(actionLockToolbars, SIGNAL(triggered()), this,
          SLOT(setToolbarAttrs())); // menu action lock toolbars
  connect(actionVisibleToolbarText, SIGNAL(triggered()), this,
          SLOT(setToolbarAttrs())); // menu action visible toolbar text
}
...
void luckyBackupWindow::createToolbar() {
  ...
  // Start & simulation
  // buttons-------------------------------------------------------------------
  profileStartToolbar->addWidget(
      ui.groupBox_operations); // profile start toolbar
...
// checkDeclared
// ===================================================================================================================================
// Check if the declared data are ok by calling checkBackupDirs, checkSyncDirs
// accordingly This is called from the gui
void luckyBackupWindow::checkDeclared() {
  int currentSelection =
      ui.listWidget_operations->currentRow(); // keep the current selection
  ...
  if (currentSelection > -1) // re-select the current selection (don't leave the
                             // last task selected)
  {
...
void luckyBackupWindow::setCurrentProfile(QString currentProfileSet) {
  if (notXnixRunning) // if OS2 or win is running and by mistake the path
                      // includes "/" instead of "\"
...
bool luckyBackupWindow::loadCurrentProfile() {
  ...
  if (currentProfile == defaultProfile)
    loadData.append(
        "<font color=blue><b>" +
        tr("loading default profile ...", "information window message") +
        "</font></b><br>");
  else
    loadData.append("<font color=blue><b>" +
                    tr("loading profile ...", "information window message") +
                    "</font></b><br>");

  int loadOK = loadProfile(currentProfile); // try to load the currentProfile
  if (loadOK == 1)                          // if it cannot open
  {
    loadData.append("<font color=red><b>" +
                    tr("loading failed", "information window message") +
                    "</font></b><br>" +
                    tr("Unable to open profile",
                       "information window message. full phrase is 'Unable to "
                       "open profile <PROFIENAME>'") +
  ...
  if (loadOK == 2) // if it is not a valid profile
  {
    loadData.append(
        "<font color=red><b>" +
        tr("loading failed", "information window message") + "</font></b><br>" +
        tr("profile ", "information window message. Full phrase is: 'profile "
                       "<PROFILENAME> is not valid for luckybackup "
                       "version:X.Y'. BEWARE of the whitespace in the end") +
        "<b>" + profileName + "</b> " +
        tr("is not valid for",
           "information window message. Full phrase is: 'profile <PROFILENAME> "
           "is not valid for luckybackup version:X.Y") +
        " " + appName + ", " +
        tr("version:",
           "information window message. Full phrase is: 'profile <PROFILENAME> "
           "is not valid for luckybackup version:X.Y") +
        countStr.setNum(appVersion));
    return false; // do nothing more
  }
  ...
  loadData.append(
      tr("profile", "info window message. full phrase: 'profile <PROFILENAME> "
                    "loaded successfully'") +
      " <b>" + profileName + "</b> <font color=green>" +
      tr("loaded successfully", "info window message. full phrase: 'profile "
                                "<PROFILENAME> loaded successfully'") +
      " !!</font>");
  ...
  {
    loadData.append("<br>" + tr("scheduled", "this refers to a profile") +
                    ": <b>");
  ...
  if (profileDescription != "") {
    ...
    loadData.append("<br><br><b>" + tr("Description") + ":</b><br>" +
                    profileDescriptionDisplay);
  }

  // Append a general directions message if the profile is empty
  if (TotalOperations == 0)
    loadData.append("<br><font color=magenta>" + tr("The task list is empty") +
                    "<br><b>" +
                    tr("Use the \"add\" button on the right to get started",
                       "Please keep the add word inside quotes") +
                    "</b></font><br>");
...
bool luckyBackupWindow::saveCurrentProfile() {
  ...
  if (currentProfile == defaultProfile)
    saveData.append(
        "<font color=blue><b>" +
        tr("saving default profile ...", "Information window message") +
        "</font></b><br>");
  else
    saveData.append("<font color=blue><b>" +
                    tr("saving profile ...", "Information window message") +
                    "</font></b><br>");

  if (!saveProfile(currentProfile)) {
    saveData.append("<font color=red><b>" + tr("WARNING") + "</b></font><br>");
    if (currentProfile == defaultProfile)
      saveData.append(tr("default profile",
                         "Information window message. Full phrase: 'default "
                         "profile <PROFILENAME> could not be saved'."));
    else
      saveData.append(
          tr("profile", "Information window message. Full phrase: 'profile "
                        "<PROFILENAME> could not be saved'"));

    saveData.append(" <b>" + profileName + "</b> " +
  ...
  saveData.append(
      tr("profile", "Information window message. Full phrase: 'profile "
                    "<PROFILENAME> saved successfully'") +
      " <b>" + profileName + "</b> <font color=green>" +
      tr("saved successfully", "Information window message. Full phrase: "
  ...
  if (cronFile.open(QIODevice::ReadOnly | QIODevice::Text)) // open the cronFile
  {
    saveData.append("<br>" + tr("scheduled", "this refers to a profile") +
                    ": <b>");
    QTextStream in(&cronFile);
    while (!in.atEnd())
      CronTab.append(in.readLine());
...
int luckyBackupWindow::createCurrentProfile() {
  createData = "";
  if (!profile
           .exists()) // if the currentProfile does not exist, try to create it
  {
    TotalOperations = 0; // Set the Operations list size to 0
    ...
    if (currentProfile == defaultProfile)
      createData.append(
          "<font color=blue><b>" +
          tr("creating default profile ...", "Information window message") +
          "</font></b><br>");
    else
      createData.append(
          "<font color=blue><b>" +
          tr("creating profile ...", "Information window message") +
          "</font></b><br>");

    if (!saveProfile(currentProfile)) // if the profile cannot be created
    {
      createData.append("<font color=red><b>" + tr("WARNING") +
                        "</b></font><br>");
      if (currentProfile == defaultProfile)
        createData.append(
            tr("default profile",
               "Information window message. Full phrase: 'default profile "
               "<PROFILENAME> could not be created'."));
      else
        createData.append(
            tr("profile", "Information window message. Full phrase: 'profile "
                          "<PROFILENAME> could not be created'."));
      createData.append(
          ...
          tr("could not be created",
             "Information window message. Full phrase: '(default) profile "
             "<PROFILENAME> could not be created'") +
          "<br>" + "<font color=red>" + profile.errorString() + "</font>");
      return 2; // profile could not be created
    }

    createData.append(
        tr("profile", "Information window message. Full phrase: 'profile "
                      "<PROFILENAME> created successfully'") +
        " <b>" + profileName + "</b> <font color=green>" +
        tr("created successfully",
           "Information window message. Full phrase: 'profile <PROFILENAME> "
           "created successfully'") +
        " !!</font>");
    createData.append("<br><font color=magenta>" +
                      tr("Use the \"add\" button on the right to get started",
                         "Please keep the add word inside quotes") +
                      "</font><br>");
...
int luckyBackupWindow::isProfileSaved() {
  if (!savedProfile) // if the current profile is not saved, ask the user if
                     // he/she wants to, before proceeding
  {
    textDialog textdialogQ(
        "QtQuestion",
        tr("Profile", "Question dialog message. Full phrase: 'profile "
                      "<PROFILENAME> is modified'") +
            " <b>" + profileName + "</b> " +
            tr("is modified", "Question dialog message. Full phrase: 'profile "
                              "<PROFILENAME> is modified'") +
            ".<br><br>" + tr("Would you like to save it before proceeding ?"),
        this);
    textdialogQ.exec();

    if (textdialogQ.getGoOn() > 0) // if user answers yes
    {
      if (!saveCurrentProfile()) // if it cannot be saved for any reason pop up
                                 // a yes/no message box to go on or not
      {
        textDialog textdialogQ2(
            "QtQuestion",
            tr("Profile", "Question dialog message. Full phrase: 'profile "
                          "<PROFILENAME> could not be saved'") +
                " <b>" + profileName + "</b> " +
                tr("could not be saved",
                   "Question dialog message. Full phrase: 'profile "
                   "<PROFILENAME> could not be saved'") +
                "<br><font color=red>" + profile.errorString() +
        ...
        {
          int previousIndex =
              ui.comboBox_profile->findText(profileName, Qt::MatchExactly);
          GoBack = true; // the next line will execute  function
                         // profileComboChanged again. set true to avoid that
...
// saveSettings
// ===================================================================================================================================
// Saves various luckybackup settings such as the default profile (text mode
// after version 0.4.7)
bool luckyBackupWindow::saveSettings() {
  QFile settingsfile(settingsFile);
  if (!settingsfile.open(QIODevice::WriteOnly)) // if the settings file cannot
                                                // be saved (or fails to create)
  {
    settingsfile.close();
    return false;
  }
...
// loadSettings
// ===================================================================================================================================
// loads various luckybackup settings such as the default profile (text mode
// after version 0.4.7)
bool luckyBackupWindow::loadSettings() {
  QFile settingsfile(settingsFile);
  if (!settingsfile.open(
          QIODevice::ReadOnly)) // if the settings file cannot be opened
  {
    settingsfile.close();
    return false;
  }
  ...
  QString SettingsLine = ""; // temp variable to import the settings line by
                             // line
  ...
  while (!(SettingsLine.startsWith("[settings_global]")) && (!in.atEnd())) {
    ...
    // --------------------------
    if (SettingsLine.startsWith("appName="))
      tempAppName = SettingsLine.remove("appName=");
    if (SettingsLine.startsWith("appVersion="))
      tempAppVersion = (SettingsLine.remove("appVersion=")).toDouble(&IntOk);
    if (SettingsLine.startsWith("File_Type="))
      tempFileType = SettingsLine.remove("File_Type=");
  }

  // check if the file is a valid luckybackup settings file
  if ((tempAppName != appName) || (tempAppVersion < validSettingsVersion) ||
      (tempFileType != "luckybackup_settings_file")) {
    settingsfile.close();
    return false;
  }
  ...
  while (!(SettingsLine.startsWith("[Settings_file_end]")) && (!in.atEnd())) {
    ...
    if (SettingsLine.startsWith("Main_window_width="))
      mainWindowWidth =
          (SettingsLine.remove("Main_window_width=")).toInt(&IntOk, 10);
    if (SettingsLine.startsWith("Main_window_height="))
      mainWindowHeight =
          (SettingsLine.remove("Main_window_height=")).toInt(&IntOk, 10);
    if (SettingsLine.startsWith("Are_toolbars_locked="))
      AreToolbarsLocked =
          (SettingsLine.remove("Are_toolbars_locked=")).toInt(&IntOk, 10);
    if (SettingsLine.startsWith("Is_combo_toolbar_visible="))
      IsVisibleProfileComboToolbar =
          (SettingsLine.remove("Is_combo_toolbar_visible=")).toInt(&IntOk, 10);
    if (SettingsLine.startsWith("Is_profile_toolbar_visible="))
      IsVisibleProfileToolbar =
          (SettingsLine.remove("Is_profile_toolbar_visible="))
              .toInt(&IntOk, 10);
    if (SettingsLine.startsWith("Is_toolbarText_visible="))
      IsVisibleToolbarText =
          (SettingsLine.remove("Is_toolbarText_visible=")).toInt(&IntOk, 10);
    if (SettingsLine.startsWith("Default_Profile="))
      defaultProfile = SettingsLine.remove("Default_Profile=");
    if (SettingsLine.startsWith("Default_Language="))
      defaultLanguage = SettingsLine.remove("Default_Language=");
    if (SettingsLine.startsWith("Visible_info_window="))
      IsVisibleInfoWindow =
          (SettingsLine.remove("Visible_info_window=")).toInt(&IntOk, 10);
    if (SettingsLine.startsWith("Quite_mode_output="))
      showOnlyErrors =
          (SettingsLine.remove("Quite_mode_output=")).toInt(&IntOk, 10);

    if (SettingsLine.startsWith("win_rsync_path="))
      rsyncCommandPath = SettingsLine.remove("win_rsync_path=");
    if (SettingsLine.startsWith("win_ssh_path="))
      sshCommandPath = SettingsLine.remove("win_ssh_path=");
    if (SettingsLine.startsWith("cygpath_path="))
      cygpathCommand = SettingsLine.remove("cygpath_path=");
    if (SettingsLine.startsWith("dosdev_path="))
      dosdevCommand = SettingsLine.remove("dosdev_path=");
    if (SettingsLine.startsWith("vshadowdir_path="))
      vshadowDir = SettingsLine.remove("vshadowdir_path=");
    if (SettingsLine.startsWith("tempdir_path=") && !isTempDirPath)
      tempDirPath = SettingsLine.remove("tempdir_path=");
  }
...
bool luckyBackupWindow::loadSettingsQV() {
  ...
  while ((vString != "Settings_file_end") && (!in.atEnd())) {
    if (vString == "Main_window_width")
      mainWindowWidth = v.toInt();
    if (vString == "Main_window_height")
      mainWindowHeight = v.toInt();
    if (vString == "Are_toolbars_locked")
      AreToolbarsLocked = v.toBool();
    if (vString == "Is_combo_toolbar_visible")
      IsVisibleProfileComboToolbar = v.toBool();
    if (vString == "Is_profile_toolbar_visible")
      IsVisibleProfileToolbar = v.toBool();
    if (vString == "Is_toolbarText_visible")
      IsVisibleToolbarText = v.toBool();
    if (vString == "Default_Profile")
      defaultProfile = v.toString();
    if (vString == "Default_Language")
      defaultLanguage = v.toString();
    if (vString == "Visible_info_window")
      IsVisibleInfoWindow = v.toBool();

    if (vString == "win_rsync_path")
      rsyncCommandPath = v.toString();
    if (vString == "win_ssh_path")
      sshCommandPath = v.toString();
...
// arrangeLogSnap
// ===================================================================================================================================
// Rename-delete-copy logs & snaps when the current profile/task is renamed,
// deleted,duplicated
//  if a task is deleted, this function is called from saveCurrentProfile()
//  Returns true if everything went ok
bool luckyBackupWindow::arrangeLogSnap(bool PorT, QString ActionTaken,
                                       QString NameToUse) {
  // PorT is "1" for profile and "0" for task
  // ActionTaken can become "rename", "delete" ,"duplicate"
  // NameToUse is the new name of the current profile if rename/duplicate or the
  // file to be deleted or the old name of the task that was renamed/deleted
  ...
  if (PorT)                               // we're talking about a profile
    ...
    filtersSnapLog << profileName + "-" + NameToUse +
                          "-*"; // NameToUse is the name of the task that was
                                // deleted/renamed (before it was renamed)

  // QSTringLists with all files to be manipulated
  QDir snapTempDir(snapChangesDir), logTempDir(logDir);
  snapListFiles = snapTempDir.entryList(
      filtersSnapLog, QDir::AllEntries | QDir::Hidden | QDir::NoDotAndDotDot,
      QDir::Name);
  logsListFiles = logTempDir.entryList(
      filtersSnapLog, QDir::AllEntries | QDir::Hidden | QDir::NoDotAndDotDot,
      QDir::Name);
  ...
  for (int count = 0; count < snapListFiles.size(); ++count) {
    ...
    // the new snap .changes filename (QString) - replace either profilename or
    // task name
    if (PorT)
      snapchangesfilename.replace(
          SLASH + profileName + "-", SLASH + NameToUse + "-",
          Qt::CaseSensitive); // QString - new filename (profile)
    else
      snapchangesfilename.replace(
          SLASH + profileName + "-" + NameToUse + "-",
          SLASH + profileName + "-" + Operation[currentOperation]->GetName() +
              "-",
          Qt::CaseSensitive); // QString - new filename (task)
    ...
    if (ActionTaken == "duplicate") // only for profile. If a task is duplicated
                                    // logs & snaps are reset
  ...
  bool whatToReturn =
      whatToReturn1 && whatToReturn2; // return true if both snaps & log actions
