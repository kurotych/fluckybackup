...
===============================================================================================================================
     ...
     luckyBackup is distributed under the terms of the GNU General Public
License luckyBackup is free software: you can redistribute it and/or modify it
...
modifyDialog::modifyDialog(int ItemNo, QDialog *parent) : QDialog(parent) {
  ...
  alsoExecuteTooltip =
      tr("Leave the box checked, to <b>STOP</b> the execution of running task "
         "if this command exits with an error");
  QString comLabel = tr("use boxes to <b>STOP</b> task execution on error",
                        "Please leave the bold tags <b></b> intact and "
                        "surrounding the word STOP translated");
  uiM.label_executeBeforeList->setText("<font color=red>" + comLabel +
                                       "</font>");
  uiM.label_executeAfterList->setText("<font color=red>" + comLabel +
                                      "</font>");

  //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  // Various gui initialization
  // uiM.toolBox_options         ->setItemEnabled (0,false);    // different
  // backend
  ...
  if (!WINrunning) {
    ...
    uiM.checkBox_vss->setEnabled(
        false); // disable VSS until a better solution is found

  //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

  // connections
  // ----------------------------------------------------------------------------------------------------
  connect(uiM.pushButton_advanced, SIGNAL(toggled(bool)), this,
          SLOT(advancedPressed(bool))); // advanced pushButton
  connect(uiM.pushButton_cancel, SIGNAL(pressed()), this,
          SLOT(cancelPressed())); // cancel pushButton
  connect(uiM.pushButton_okay, SIGNAL(pressed()), this,
          SLOT(okayPressed())); // okay pushButton
  connect(uiM.pushButton_command, SIGNAL(pressed()), this,
          SLOT(commandPressed())); // show command pushButton
  connect(uiM.comboBox_Type, SIGNAL(currentIndexChanged(int)), this,
          SLOT(TaskTypeChanged(
              int))); // Actions when task type combo changes (hide/show stuff)

  // Connect all include tab stuff changes with the disableExcludeTab SLOT
  connect(uiM.checkBox_includeFile, SIGNAL(stateChanged(int)), this,
          SLOT(disableExcludeTab()));
  connect(uiM.lineEdit_includeFile, SIGNAL(textChanged(QString)), this,
          SLOT(disableExcludeTab()));
  connect(uiM.radioButton_includeOnly, SIGNAL(toggled(bool)), this,
          SLOT(disableExcludeTab()));
  connect(uiM.radioButton_includeNormal, SIGNAL(toggled(bool)), this,
          SLOT(disableExcludeTab()));

  // Connect all add list items buttons with the addListItem SLOT
  // Map all the pushButton signals and transmit a different integer depending
  // on the pushbutton
  QSignalMapper *signalMapper2 = new QSignalMapper(this);
  signalMapper2->setMapping(uiM.pushButton_excludeAdd, 0);
  connect(uiM.pushButton_excludeAdd, SIGNAL(pressed()), signalMapper2,
          SLOT(map())); // excludAdd pushButton
  signalMapper2->setMapping(uiM.pushButton_includeAdd, 1);
  connect(uiM.pushButton_includeAdd, SIGNAL(pressed()), signalMapper2,
          SLOT(map())); // includeAdd pushButton
  signalMapper2->setMapping(uiM.pushButton_optionsAdd, 2);
  connect(uiM.pushButton_optionsAdd, SIGNAL(pressed()), signalMapper2,
          SLOT(map())); // optionsAdd pushButton
  signalMapper2->setMapping(uiM.pushButton_executeBeforeAdd, 3);
  connect(uiM.pushButton_executeBeforeAdd, SIGNAL(pressed()), signalMapper2,
          SLOT(map())); // executeBeforeAdd pushButton
  signalMapper2->setMapping(uiM.pushButton_executeAfterAdd, 4);
  connect(uiM.pushButton_executeAfterAdd, SIGNAL(pressed()), signalMapper2,
          SLOT(map())); // executeAftereAdd pushButton
  connect(signalMapper2, SIGNAL(mapped(const int)), this,
          SLOT(addListItem(const int)));
  ...
  connect(uiM.pushButton_excludeRemove, SIGNAL(pressed()), signalMapper3,
          SLOT(map())); // excludeRemove pushButton
  signalMapper3->setMapping(uiM.pushButton_includeRemove, 1);
  connect(uiM.pushButton_includeRemove, SIGNAL(pressed()), signalMapper3,
          SLOT(map())); // includeRemove pushButton
  signalMapper3->setMapping(uiM.pushButton_optionsRemove, 2);
  connect(uiM.pushButton_optionsRemove, SIGNAL(pressed()), signalMapper3,
          SLOT(map())); // optionsRemove pushButton
  signalMapper3->setMapping(uiM.pushButton_executeBeforeRemove, 3);
  connect(uiM.pushButton_executeBeforeRemove, SIGNAL(pressed()), signalMapper3,
          SLOT(map())); // executeBeforeRemove pushButton
  signalMapper3->setMapping(uiM.pushButton_executeAfterRemove, 4);
  connect(uiM.pushButton_executeAfterRemove, SIGNAL(pressed()), signalMapper3,
          SLOT(map())); // executeAfterRemove pushButton
  connect(signalMapper3, SIGNAL(mapped(const int)), this,
          SLOT(removeListItem(const int)));
  ...
  connect(uiM.pushButton_editExclude, SIGNAL(pressed()), signalMapper5,
          SLOT(map())); // EditExclude pushButton
  signalMapper5->setMapping(uiM.pushButton_editInclude, 1);
  connect(uiM.pushButton_editInclude, SIGNAL(pressed()), signalMapper5,
          SLOT(map())); // EditInclude pushButton
  signalMapper5->setMapping(uiM.pushButton_editOptions, 2);
  connect(uiM.pushButton_editOptions, SIGNAL(pressed()), signalMapper5,
          SLOT(map())); // EditOptiions pushButton
  signalMapper5->setMapping(uiM.pushButton_editExecuteBefore, 3);
  connect(uiM.pushButton_editExecuteBefore, SIGNAL(pressed()), signalMapper5,
          SLOT(map())); // EditExecuteBefore pushButton
  signalMapper5->setMapping(uiM.pushButton_editExecuteAfter, 4);
  connect(uiM.pushButton_editExecuteAfter, SIGNAL(pressed()), signalMapper5,
          SLOT(map())); // EditExecuteAfter pushButton
  connect(signalMapper5, SIGNAL(mapped(const int)), this,
          SLOT(editListItem(const int)));
  ...
  connect(uiM.pushButton_moveUpExclude, SIGNAL(pressed()), signalMapper6,
          SLOT(map()));
  signalMapper6->setMapping(uiM.pushButton_moveUpInclude, 1);
  connect(uiM.pushButton_moveUpInclude, SIGNAL(pressed()), signalMapper6,
          SLOT(map()));
  signalMapper6->setMapping(uiM.pushButton_moveUpOptions, 2);
  connect(uiM.pushButton_moveUpOptions, SIGNAL(pressed()), signalMapper6,
          SLOT(map()));
  signalMapper6->setMapping(uiM.pushButton_moveUpExecuteBefore, 3);
  connect(uiM.pushButton_moveUpExecuteBefore, SIGNAL(pressed()), signalMapper6,
          SLOT(map()));
  signalMapper6->setMapping(uiM.pushButton_moveUpExecuteAfter, 4);
  connect(uiM.pushButton_moveUpExecuteAfter, SIGNAL(pressed()), signalMapper6,
          SLOT(map()));
  connect(signalMapper6, SIGNAL(mapped(const int)), this,
          SLOT(moveListItemUp(const int)));
  ...
  connect(uiM.pushButton_moveDownExclude, SIGNAL(pressed()), signalMapper7,
          SLOT(map()));
  signalMapper7->setMapping(uiM.pushButton_moveDownInclude, 1);
  connect(uiM.pushButton_moveDownInclude, SIGNAL(pressed()), signalMapper7,
          SLOT(map()));
  signalMapper7->setMapping(uiM.pushButton_moveDownOptions, 2);
  connect(uiM.pushButton_moveDownOptions, SIGNAL(pressed()), signalMapper7,
          SLOT(map()));
  signalMapper7->setMapping(uiM.pushButton_moveDownExecuteBefore, 3);
  connect(uiM.pushButton_moveDownExecuteBefore, SIGNAL(pressed()),
          signalMapper7, SLOT(map()));
  signalMapper7->setMapping(uiM.pushButton_moveDownExecuteAfter, 4);
  connect(uiM.pushButton_moveDownExecuteAfter, SIGNAL(pressed()), signalMapper7,
          SLOT(map()));
  connect(signalMapper7, SIGNAL(mapped(const int)), this,
          SLOT(moveListItemDown(const int)));

  // Connect all toolButtons with the browse SLOT
  // Map all the toolButton signals and transmit a different integer depending
  // on the toolbutton
  QSignalMapper *signalMapper = new QSignalMapper(this);
  signalMapper->setMapping(uiM.toolButton_sourceBrowse, 0);
  connect(uiM.toolButton_sourceBrowse, SIGNAL(clicked()), signalMapper,
          SLOT(map()));
  signalMapper->setMapping(uiM.toolButton_destinationBrowse, 1);
  connect(uiM.toolButton_destinationBrowse, SIGNAL(clicked()), signalMapper,
          SLOT(map()));
  signalMapper->setMapping(uiM.toolButton_rsyncPassword, 4);
  connect(uiM.toolButton_rsyncPassword, SIGNAL(clicked()), signalMapper,
          SLOT(map()));
  signalMapper->setMapping(uiM.toolButton_sshPassword, 5);
  connect(uiM.toolButton_sshPassword, SIGNAL(clicked()), signalMapper,
          SLOT(map()));
  signalMapper->setMapping(uiM.toolButton_executeBeforeBrowse, 6);
  connect(uiM.toolButton_executeBeforeBrowse, SIGNAL(clicked()), signalMapper,
          SLOT(map()));
  signalMapper->setMapping(uiM.toolButton_executeAfterBrowse, 7);
  connect(uiM.toolButton_executeAfterBrowse, SIGNAL(clicked()), signalMapper,
          SLOT(map()));
  signalMapper->setMapping(uiM.toolButton_includeFile, 8);
  connect(uiM.toolButton_includeFile, SIGNAL(clicked()), signalMapper,
          SLOT(map()));
  signalMapper->setMapping(uiM.toolButton_excludeFile, 9);
  connect(uiM.toolButton_excludeFile, SIGNAL(clicked()), signalMapper,
          SLOT(map()));
  connect(signalMapper, SIGNAL(mapped(const int)), this,
          SLOT(browse(const int)));
  ...
  connect(uiM.toolButton_excludeAddPattern, SIGNAL(clicked()), signalMapper4,
          SLOT(map()));
  signalMapper4->setMapping(uiM.toolButton_includeAddPattern, 1);
  connect(uiM.toolButton_includeAddPattern, SIGNAL(clicked()), signalMapper4,
          SLOT(map()));
  connect(signalMapper4, SIGNAL(mapped(const int)), this,
          SLOT(getPattern(const int)));

  // ssh password file default value
  // I do not consider this a good idea, because the default option should be a
  // blank field (no ssh key file) If the user clicks the browse button , he/she
  // will be transfered to the ~/.ssh dir as default
  /*
  if ( (uiM.lineEdit_sshPassword -> text() == "" ) && (!WINrunning) )
  {
      if (!WINrunning)
          uiM.lineEdit_sshPassword -> setText( myHome + SLASH +".ssh" + SLASH
  +"id_rsa"); else uiM.lineEdit_sshPassword -> setText(luckyBackupDir+"id_rsa");
  }
  */

  if (ArrayPosition < TotalOperations) // if modify is pressed fill the fields
                                       // first with existing operation data
    fillModifyWindow(Operation[ArrayPosition]);

  disableExcludeTab(); // disable the exclude tab or nothing

  StdArguments << "-h" << "--progress"
...
// window closebutton pressed or QcloseEvent
// emitted==========================================================================
void modifyDialog::closeEvent(QCloseEvent *event) { event->accept(); }
...
void modifyDialog::browse(const int type) {
  ...
  case 0:
    newLineEdit = QFileDialog::getExistingDirectory(
        this, tr("Select source directory", "file selection dialog title"),
        uiM.lineEdit_source->text());
  ...
  case 1:
    newLineEdit = QFileDialog::getExistingDirectory(
        this, tr("Select destination directory", "file selection dialog title"),
        uiM.lineEdit_destination->text());
    if (newLineEdit != "") // if something is selected indeed
      uiM.lineEdit_destination->setText(modifyOS2Slashes(newLineEdit));
    break;
  // rsync password file browse button
  case 4:
    newLineEdit = QFileDialog::getOpenFileName(
        this, tr("Select rsync password file", "file selection dialog title"),
        myHome);
    if (newLineEdit != "") // if something is selected indeed
  ...
  case 5:
    if (!WINrunning)
      newLineEdit = QFileDialog::getOpenFileName(
          this, tr("Select ssh keyfile", "file selection dialog title"),
          myHome + SLASH + ".ssh");
    else
      newLineEdit = QFileDialog::getOpenFileName(
          this, tr("Select ssh keyfile", "file selection dialog title"),
  ...
  case 6:
    newLineEdit = QFileDialog::getOpenFileName(
        this,
        tr("Select command to be executed", "file selection dialog title"),
        SLASH);
    if (newLineEdit != "") // if something is selected indeed
  ...
  case 8:
    newLineEdit =
        QFileDialog::getOpenFileName(this,
                                     tr("Select file to read include patterns",
  ...
  case 9:
    newLineEdit =
        QFileDialog::getOpenFileName(this,
                                     tr("Select file to read exclude patterns",
                                        "file selection dialog title"),
                                     myHome);
    if (newLineEdit != "") // if something is selected indeed
...
void modifyDialog::getPattern(const int type) {
  if (uiM.lineEdit_source->text() == "") // if the source isn't defined yet
  {
    textDialog textdialogW(
        "QtWarning",
        tr("Please define the source before launching the pattern editor",
           "message box, message") +
            " !!",
        this);
    textdialogW.exec();
  ...
  modifyTrailing(); // first modify the trailing of the source & destination
                    // directories
  ...
  if (NewPattern ==
      "no pattern defined yet") // cancel pressed at the pattern editor
    return;
  switch (type) {
  // insert the pattern to the exclude lineEdit
  case 0:
    uiM.lineEdit_excludeAdd->setText(NewPattern);
    break;
  // insert the pattern to the incude lineEdit
  case 1:
    uiM.lineEdit_includeAdd->setText(NewPattern);
    break;
  }
}
// okay button
...
void modifyDialog::okayPressed() {
  ...
  if ((uiM.lineEdit_name->text() == "") ||
    ...
    message.append(/*"<b>"+tr("ERROR","message box message")+":</b> "+*/
                   tr("Please specify the following before proceeding:",
                      "message box, message"));
    if (uiM.lineEdit_name->text() == "")
  ...
  // ------------------------------------------------
  if ((uiM.lineEdit_source->text() == "/") &&
      (uiM.comboBox_Type->currentIndex() == 0) &&
      (!uiM.checkBox_backupContents->isChecked())) {
    modifyOK = false;
    message
        .append(/*"<b>"+tr("ERROR","message box message - line1")+":</b> "+*/
                tr("You have declared the '/' (root) directory as source and "
                   "have NOT checked the 'Do NOT create extra directory' box",
                   "message box message - line1") +
                "<br>" +
                tr("That is not allowed because '/' does not have an actual "
                   "name",
                   "message box message - line2") +
                " !!<br>"
                "..." +
                tr("Please check the relevant box before proceeding",
                   "message box message - line3") +
                "<br><br>");
  }
  // same for OS2 or win
  if (((uiM.lineEdit_source->text()).endsWith(":\\")) &&
      (uiM.comboBox_Type->currentIndex() == 0) &&
      (!uiM.checkBox_backupContents->isChecked()) && (notXnixRunning)) {
    modifyOK = false;
    message.append(/*"<b>"+tr("ERROR","message box message - line1")+":</b> "+*/
                   tr("You have declared the root directory of an entire "
                      "partition as source and have NOT checked the 'Do NOT "
                      "create extra directory' box",
                      "message box message - line1") +
                   "<br>" +
                   tr("That is not allowed because the root partition does not "
                      "have an actual name",
                      "message box message - line2") +
                   " !!<br>"
                   "..." +
  ...
  while (count < TotalOperations) {
    if (((uiM.lineEdit_name->text()) == (Operation[count]->GetName())) &&
        (count != ArrayPosition)) {
      modifyOK = false;
      message
          .append(/*"<b>"+tr("ERROR","message box message - line1")+":</b> "+*/
                  tr("The name of the task you requested already exists",
                     "message box message - line1") +
                  " !!<br>"
                  "..." +
                  tr("Please specify a different name before proceeding",
                     "message box message - line2") +
                  "<br><br>");
      break;
    }
  ...
  modifyTrailing(); // do some changes according to radio button "operation
                    // type"
  ...
  if (((uiM.lineEdit_source->text()) != "") &&
    ...
    if ((((uiM.lineEdit_source->text()) ==
          (uiM.lineEdit_destination->text())) ||
         ((uiM.lineEdit_source->text() + SLASH) ==
          (uiM.lineEdit_destination->text()))) &&
        (!uiM.groupBox_remote->isChecked())) {
      modifyOK = false;
      message
          .append(/*"<b>"+tr("ERROR","message box message - line1")+":</b> "+*/
                  tr("The source you have declared is identical to the "
                     "destination",
                     "message box message - line1") +
                  " !!<br>"
                  "..." +
                  tr("Please modify at least one of them",
                     "message box message - line2") +
                  "<br><br>");
    } else // check if the destination is part of the source directory structure
           // ------------------------------------------------
      if (((uiM.lineEdit_destination->text())
               .contains(uiM.lineEdit_source->text(), Qt::CaseSensitive)) ||
          ((uiM.lineEdit_destination->text())
               .contains(uiM.lineEdit_source->text() + SLASH,
                         Qt::CaseSensitive)))
        message
            .prepend(/*"<b>"+tr("WARNING","message box message - line1")+":</b>
                        "+*/
                     tr("You have declared a destination that is part of the "
                        "source directory structure",
                        "message box message - line1") +
                     "<br>" +
                     tr("<b>Do not forget</b> to define a directory at the "
                        "'exclude' groupbox that will contain the destination",
                        "message box message - line2. Please leave tag "
                        "<b>,</b> intact and surrounding 'do not forget' "
                        "translated") +
                     "<br><br>");
  }
  ...
  //-------------------------------------------------------------
  if ((uiM.checkBox_deleteAfter->isChecked()) &&
      !(uiM.checkBox_recurse->isChecked())) {
    modifyOK = false;
    message.append(/*"<b>"+tr("ERROR","message box message - line1")+":</b> "+*/
                   tr("You have checked the 'delete files on the destination' "
                      "option",
                      "message box message - line2") +
                   "<br>" +
                   tr("without having checked 'Recurse into directories' which "
                      "is required",
                      "message box message - line3") +
                   "<br><br>");
  }
  ...
  // -----------------------------------------------------------------
  if ((uiM.groupBox_remote->isChecked()) &&
      (uiM.lineEdit_remoteHost->text() == "")) {
    modifyOK = false;
    message.append(/*"<b>"+tr("ERROR","message box message - line1")+":</b>"+*/
                   tr("'Remote Host' is checked, but no remote host name is "
                      "declared",
                      "message box message - line2") +
                   "<br>" +
                   tr("Please specify a remote host name",
                      "message box message - line3") +
                   "<br>" +
                   tr("Otherwise uncheck 'Remote Host' to operate locally",
                      "message box message - line4") +
                   "<br><br>");
  }

  bool tempCloneWarning = true;
  // use this cause the next command will set CloneWARNING to true by creating a
  // new operation
  ...
  Operation[ArrayPosition] =
      fillOperationArray(); // fill the operation array item with data from the
                            // modify dialog
  ...
  if (!validation) // if "done" pressed and not "validation check"
  {
    ...
    {
      // check if the user has selected "delete files on the destination" &
      // "backup the contents of a source directory" and WARN
      if ((uiM.comboBox_Type->currentIndex() == 0) &&
          (uiM.checkBox_deleteAfter->isChecked()) &&
          (Operation[ArrayPosition]->GetCloneWARNING()) &&
          (uiM.checkBox_backupContents->isChecked())) {
        textDialog textdialog("CloneWarning", "", this);
        textdialog.exec();
        if (textdialog.getGoOn() ==
            4) // okay with "Do not show this again" checked
        {
          Operation[ArrayPosition]->SetCloneWARNING(false);
          close();
        }
        if (textdialog.getGoOn() ==
            2)     // okay with "Do not show this again" NOT checked
          close(); // emmit a QcloseEvent
      } else
        close(); // emmit a QcloseEvent
...
// validate button
// pressed=======================================================================================================
// Pops up a dialog with the rsync command & arguments, also displays error
// messages
void modifyDialog::commandPressed() {
  ...
  QStringList arguments = Operation[ArrayPosition]->GetArgs();
  QString tempArgs = "";
  count = 0;
  QString source, dest;
  ...
  if (modifyOK)
    message.prepend(
        tr("All declared fields of the task properties window seem to be ok",
           "message box message first line") +
        "<br><br>");
  else
    message.prepend(
        "<b>" + tr("Errors have been found", "message box message first line") +
        "...</b><br><br>");

  message.append("<b>" +
                 tr("command to be executed",
                    "full phrase: command to be executed: <COMMAND>") +
                 ":</b><br>");

  textDialog textdialog(
      "ValidateDialog",
      message + "<font color=red>rsync </font><font color=blue>" + tempArgs +
          "</font><br><font color=magenta>" + source +
          " </font><br><font color=magenta>" + dest + "</font><br>",
      this);
  textdialog.exec();
...
void modifyDialog::addListItem(const int type) {
  ...
  case 0:
    ...
    if (uiM.listWidget_exclude->count() ==
        0) // if this is the first entry, just add it (to avoid segfault)
    {
    ...
    if (listItemEditMode)
      selectedText =
          (uiM.listWidget_exclude->currentItem())
              ->text(); // The selected list item text (used for listItemEdit)
    temp = uiM.listWidget_exclude->findItems(
        text, Qt::MatchExactly); // list with all other items that are the same
                                 // with the lineedit

    // If the text in the lineedit already exists
    if ((temp.size() > 0) && (text != selectedText)) {
      textDialog textdialogw("QtWarning",
                             tr("You have already added this exclude item",
                                "message box message") +
                                 " !!<br>",
                             this);
      textdialogw.exec();
    ...
    if (listItemEditMode) {
      ...
      uiM.listWidget_exclude->takeItem(
          selectedItem); // take out the selected item from the list
      uiM.listWidget_exclude->insertItem(
          selectedItem,
          text); // insert a new one at the same position with the new text
  ...
  case 1:
    ...
    if (uiM.listWidget_include->count() ==
        0) // if this is the first entry, just add it (to avoid segfault)
    {
    ...
    if (listItemEditMode)
      selectedText =
          (uiM.listWidget_include->currentItem())
              ->text(); // The selected list item text (used for listItemEdit)
    temp = uiM.listWidget_include->findItems(
        text, Qt::MatchExactly); // list with all other items that are the same
                                 // with the lineedit
    ...
    if ((temp.size() > 0) && (text != selectedText)) {
      textDialog textdialogw("QtWarning",
                             tr("You have already added this include item",
                                "message box message") +
                                 " !!<br>",
                             this);
      textdialogw.exec();
    ...
    if (listItemEditMode) {
      ...
      uiM.listWidget_include->takeItem(
          selectedItem); // take out the selected item from the list
      uiM.listWidget_include->insertItem(
          selectedItem,
          text); // insert a new one at the same position with the new text
  ...
  case 2:
    ...
    if (text == "") // if there is nothing typed just return
      return;
    if (uiM.listWidget_options->count() ==
        0) // if this is the first entry, just add it (to avoid segfault)
    {
    ...
    if (listItemEditMode)
      selectedText =
          (uiM.listWidget_options->currentItem())
              ->text(); // The selected list item text (used for listItemEdit)
    temp = uiM.listWidget_options->findItems(
        text, Qt::MatchExactly); // list with all other items that are the same
                                 // with the lineedit
    ...
    if ((temp.size() > 0) && (text != selectedText)) {
      textDialog textdialogW(
          "QtWarning",
          tr("You have already added this option", "message box message") +
              " !!<br>",
          this);
      textdialogW.exec();
      return;
    ...
    if (listItemEditMode) {
      ...
      uiM.listWidget_options->takeItem(
          selectedItem); // take out the selected item from the list
      uiM.listWidget_options->insertItem(
          selectedItem,
          text); // insert a new one at the same position with the new text
  ...
  case 3:
    ...
    if (text != "") {
      if (listItemEditMode) {
        ...
        uiM.listWidget_executeBefore->takeItem(
            selectedItem); // take out the selected item from the list
                           // insert a new one at the same position with the new
                           // text
        uiM.listWidget_executeBefore->insertItem(selectedItem, text);
        uiM.listWidget_executeBefore->setCurrentRow(
            selectedItem); // highlight this row
        uiM.listWidget_executeBefore->currentItem()->setCheckState(
            alsoExecuteState); // set its state to what it was
        uiM.listWidget_executeBefore->currentItem()->setToolTip(
            alsoExecuteTooltip); // set the tooltip

        // set the gui back to its original form
        uiM.pushButton_executeBeforeAdd->setText(origAddText);
        uiM.pushButton_executeBeforeAdd->setToolTip(origAddToolTip);
        uiM.pushButton_executeBeforeAdd->setIcon(
            QIcon(":/luckyPrefix/add.png"));
        uiM.pushButton_executeBeforeRemove->setText(origRemoveText);
        uiM.pushButton_executeBeforeRemove->setToolTip(origRemoveToolTip);
        uiM.pushButton_executeBeforeRemove->setIcon(
            QIcon(":/luckyPrefix/remove.png"));
        ...
        uiM.listWidget_executeBefore->setCurrentRow(
            uiM.listWidget_executeBefore->count() - 1);
  ...
  case 4:
    ...
    if (text != "") {
      if (listItemEditMode) {
        ...
        uiM.listWidget_executeAfter->takeItem(
            selectedItem); // take out the selected item from the list
                           // insert a new one at the same position with the new
                           // text
        uiM.listWidget_executeAfter->insertItem(selectedItem, text);
        uiM.listWidget_executeAfter->setCurrentRow(
            selectedItem); // highlight this row
        uiM.listWidget_executeAfter->currentItem()->setCheckState(
            alsoExecuteState); // set its state to what it was
        uiM.listWidget_executeAfter->currentItem()->setToolTip(
            alsoExecuteTooltip);
        ...
        uiM.pushButton_executeAfterRemove->setIcon(
            QIcon(":/luckyPrefix/remove.png"));
        ...
        uiM.listWidget_executeAfter->setCurrentRow(
            uiM.listWidget_executeAfter->count() - 1);
...
void modifyDialog::removeListItem(const int type) {
  ...
  case 0:
    if (listItemEditMode) // do not proceed further if this is just a cancel
                          // from list item edit mode
    {
      // set the gui back to its original form
      uiM.pushButton_excludeAdd->setText(origAddText);
      uiM.pushButton_excludeAdd->setToolTip(origAddToolTip);
      ...
      uiM.pushButton_moveDownExclude->setEnabled(true);
      listItemEditMode =
          false; // set this bool var to false - end of list edit mode
      return;
    }
  ...
  case 3:
    ...
    selected =
        uiM.listWidget_executeBefore->currentRow(); // current list row number
    if (selected < 0) // if nothing is selected do nothing
      return;
  ...
  case 4:
    ...
    selected =
        uiM.listWidget_executeAfter->currentRow(); // current list row number
    if (selected < 0) // if nothing is selected do nothing
      return;
...
void modifyDialog::editListItem(const int type) {
  ...
  case 0:
    ...
    selectedText = (uiM.listWidget_exclude->currentItem())
                       ->text(); // current list item text
  ...
  case 1:
    ...
    selectedText = (uiM.listWidget_include->currentItem())
                       ->text(); // current list item text
  ...
  case 2:
    ...
    selectedText = (uiM.listWidget_options->currentItem())
                       ->text(); // current list item text
  ...
  case 3:
    ...
    selectedText = (uiM.listWidget_executeBefore->currentItem())
                       ->text(); // current list item text
    alsoExecuteState = uiM.listWidget_executeBefore->currentItem()
                           ->checkState(); // current list item check state
  ...
  case 4:
    ...
    selectedText = (uiM.listWidget_executeAfter->currentItem())
                       ->text(); // current list item text
    alsoExecuteState = uiM.listWidget_executeAfter->currentItem()
                           ->checkState(); // current list item check state
...
void modifyDialog::moveListItemUp(const int type) {
  ...
  case 0:
    ...
    if (selected < 0) // if nothing is selected do nothing
      return;
    if (selected >
        0) // if this is not the first row proceed with swapping items
    {
...
void modifyDialog::moveListItemDown(const int type) {
  ...
  case 0:
    ...
    if (selected <
        totalListItems -
            1) // if this is not the last row proceed with swapping items
    {
      tempItem = uiM.listWidget_exclude->takeItem(selected);
      uiM.listWidget_exclude->insertItem(selected + 1, tempItem);
      uiM.listWidget_exclude->setCurrentRow(selected + 1);
  ...
  case 3:
    selected =
        uiM.listWidget_executeBefore->currentRow(); // current list row number
    totalListItems =
        uiM.listWidget_executeBefore->count(); // total no of list items
    if (selected < 0) // if nothing is selected do nothing
      return;
    if (selected <
        totalListItems -
  ...
  case 4:
    selected =
        uiM.listWidget_executeAfter->currentRow(); // current list row number
    totalListItems =
        uiM.listWidget_executeAfter->count(); // total no of list items
    if (selected < 0) // if nothing is selected do nothing
      return;
    if (selected <
        totalListItems -
...
void modifyDialog::disableExcludeTab() {
  ...
  if (((uiM.listWidget_include->count() > 0) ||
       ((uiM.checkBox_includeFile->isChecked()) &&
        ((uiM.lineEdit_includeFile->text()).size() > 0))) &&
...
operation *modifyDialog::fillOperationArray() {
  ...
  // Before doing any changes, inherit last execution time & error as well as
  // snapshots list from the existing task if this is not a new task (modify
  // task. NOT new/duplicate/restore task)
  if (!newTaskThisIs) {
    pTask->SetLastExecutionTime(
        Operation[ArrayPosition]->GetLastExecutionTime());
    pTask->SetLastExecutionErrors(
        Operation[ArrayPosition]->GetLastExecutionErrors());
  ...
  if (uiM.comboBox_Type->currentIndex() == 1) {
    ...
    uiM.spinBox_snapshotsKeep->setValue(
        1); // also set the keep snapshots spinbox to "1" if it is a sync task
  }
  ...
  // if ( (uiM.groupBox_remote -> isChecked()) &&
  // (uiM.radioButton_remoteDestination -> isChecked()) )
  ...
  while (count < (uiM.listWidget_executeBefore->count())) {
    ...
    pTask->AddExecuteBeforeListItem(
        (uiM.listWidget_executeBefore->currentItem())->text());
    if ((uiM.listWidget_executeBefore->currentItem())->checkState() ==
        Qt::Checked)
      tempState = true;
    else
      tempState = false;
  ...
  while (count < (uiM.listWidget_executeAfter->count())) {
    ...
    pTask->AddExecuteAfterListItem(
        (uiM.listWidget_executeAfter->currentItem())->text());
    if ((uiM.listWidget_executeAfter->currentItem())->checkState() ==
        Qt::Checked)
      tempState = true;
    else
      tempState = false;
...
void modifyDialog::fillModifyWindow(operation *pTask) {
  ...
  while (count < (pTask->GetExecuteBeforeListSize())) {
    uiM.listWidget_executeBefore->addItem(
        pTask->GetExecuteBeforeListItem(count));
    uiM.listWidget_executeBefore->setCurrentRow(
        uiM.listWidget_executeBefore->count() - 1);
...
void modifyDialog::modifyTrailing() {
  ...
  // Check if a drive letter is declared at OS/2 or win (eg c:) and add a
  // trailing slash
  if (notXnixRunning) {
  ...
  if ((uiM.comboBox_Type->currentIndex() == 0) &&
      (uiM.checkBox_backupContents->isChecked())) {
    if ((!sourceDir.endsWith(XnixSLASH)) && (!sourceDir.endsWith(SLASH)) &&
        (sourceDir != "")) {
      if (uiM.groupBox_remote->isChecked() &&
          uiM.radioButton_remoteSource->isChecked())
        sourceDir.append(XnixSLASH);
      else
        sourceDir.append(SLASH);
    }
    if ((!destDir.endsWith(XnixSLASH)) && (!destDir.endsWith(SLASH)) &&
        (destDir != "")) {
      if (uiM.groupBox_remote->isChecked() &&
          uiM.radioButton_remoteDestination->isChecked())
  ...
  if ((uiM.comboBox_Type->currentIndex() == 0) &&
    ...
    {
      if ((sourceDir.endsWith(SLASH)) && (!sourceDir.endsWith(":" + SLASH)))
        sourceDir.chop(1);
      if ((!destDir.endsWith(XnixSLASH)) && (!destDir.endsWith(SLASH)) &&
          (destDir != "")) {
        if (uiM.groupBox_remote->isChecked() &&
    ...
    {
      if ((sourceDir.endsWith(SLASH)) && (sourceDir != SLASH))
        sourceDir.chop(1);
      if ((!destDir.endsWith(SLASH)) && (destDir != ""))
        destDir.append(SLASH);
    }
  }

  if (uiM.comboBox_Type->currentIndex() ==
      1) // sync Directories - both paths have to have a trailing slash
  {
    if ((!sourceDir.endsWith(XnixSLASH)) && (!sourceDir.endsWith(SLASH)) &&
        (sourceDir != "")) {
      if (uiM.groupBox_remote->isChecked() &&
  ...
  /*if (WINrunning)     // some more adjustments for windows when the user wants
  to declare directly paths with "/" instead of "\" (normally for remote)
  {
