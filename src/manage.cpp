...
manageDialog::manageDialog(QDialog *parent) : QDialog(parent) {
  ...
  {
    sourceLast =
        calculateLastPath(source); // This is the lowest dir of the source
  ...
  connect(calcdiffthread, SIGNAL(updateViewer(const QString, const bool)), this,
          SLOT(refreshDifWindow(const QString &, const bool &)));
  ...
  connect(uiG.pushButton_restore, SIGNAL(clicked()), this,
          SLOT(restoreBackup()));
  connect(uiG.pushButton_delete, SIGNAL(clicked()), this, SLOT(deleteBackup()));
  connect(uiG.checkBox_viewSource, SIGNAL(clicked()), this,
          SLOT(hideSourceStuff()));
  connect(uiG.listWidget_dates, SIGNAL(itemSelectionChanged()), this,
          SLOT(SnapshotsListSelected()));
  connect(uiG.pushButton_calculate_diffs, SIGNAL(clicked()), this,
          SLOT(calculateDifferences()));

  //____________________Initial gui changes (these will remain the same for as
  //long as the gui is visible)_______________

  // set the text to labels profile & task
  uiG.label_profile->setText(tr("profile") + ": <font color = blue><b>" +
                             profileName + "</b></font>");
  uiG.label_task->setText(tr("task") + ": <font color = blue><b>" +
  ...
  (uiG.textBrowser_snapshot->document())
      ->setMaximumBlockCount(50000); // set maximum blocks to avoid freeze

  updateSnapshots();       // update the snapshots listWidget
  SnapshotsListSelected(); // set currentSnapshotString & No. Will also call
                           // fixGui()
}
// SLOTS-------------------------------------------------------------------------------------
// --------------------------------cancel
// pressed------------------------------------------------
void manageDialog::cancel() { close(); }
...
void manageDialog::logView() {
  ...
  logfilename.prepend(logDir + profileName + "-" +
                      Operation[currentOperation]->GetName() + "-");
...
void manageDialog::restoreBackup() {
  manageWizard restoreWizard(
      "restoreBackup", source, dest,
      CurrentSnapshotNo); // pass the number of the current row
  restoreWizard.exec();
...
void manageDialog::fixGui() {
  ...
  // set the model of the treebrowsers (source & destination) sort by name &
  // refresh to use new source & dest
  // destination
  QDirModel *d_model = new QDirModel;

  // d_model -> setFilter(QDir::AllDirs | QDir::NoDotAndDotDot |
  // QDir::AllEntries | QDir::Hidden); d_model -> setSorting(QDir::Name |
  ...
  //	uiG.treeView_browser ->
  //setRootIndex(d_model->index(CurrentSnapshotDirectory)); else
  ...
  // s_model -> setFilter(QDir::AllDirs | QDir::NoDotAndDotDot |
  // QDir::AllEntries | QDir::Hidden); s_model -> setSorting(QDir::Name |
  // QDir::DirsFirst | QDir::IgnoreCase | QDir::LocaleAware);
  ...
  if (CurrentSnapshotString == "") {
    ...
    if (!Operation[currentOperation]->GetTypeSync()) {
      destVisible = false;
      uiG.label_destError->setText(
          "<font color=magenta>" +
          tr("Please select a snapshot from the list", "Information message") +
          "</font>");
    }
  }
  ...
  // _______________________________ source & destination directories checks
  // ____________________________________________
  // ___________________________________________________________________________________________________________________
  // check if source & destination exist or have read/enter permissions or are
  // remote ----------------------------------
  QFileInfo destFile(dest);
  QFileInfo sourceFile(source);
  bool sourceRemote = (Operation[currentOperation]->GetRemote()) &&
                      (Operation[currentOperation]->GetRemoteSource());
  bool destRemote = (Operation[currentOperation]->GetRemote()) &&
                    (Operation[currentOperation]->GetRemoteDestination());

  if (sourceRemote) // If remote is used for source
  {
    // sourceVisible = false;
    // uiG.label_sourceError -> setText("<font color=magenta>" + tr("Display of
    // remote places is not supported",
    ...
    s_model->setFilter(QDir::AllDirs | QDir::NoDotAndDotDot | QDir::AllEntries |
                       QDir::Hidden);
    s_model->setSorting(QDir::Name | QDir::DirsFirst | QDir::IgnoreCase |
                        QDir::LocaleAware);
    uiG.treeView_source->setModel(s_model);
    uiG.treeView_source->sortByColumn(0, Qt::AscendingOrder);
    uiG.treeView_source->setRootIndex(s_model->index(source));
  ...
  if (destRemote) // If remote is used for destination
  {
    ...
    // ********************** WARNING - Change this when you figure out a way to
    // display/delete remote places *************************************
    //        uiG.label_destError -> setText("<font color=magenta>" +
    //        tr("Display of remote places is not supported",
    //                                        "Information message") +
    //                                        "</font>");
  ...
  } else // Local destination
  {
    // if (CurrentSnapshotDir.exists())
    //  uiG.treeView_browser ->
    //  setRootIndex(d_model->index(CurrentSnapshotDirectory));
    // else

    d_model->setFilter(QDir::AllDirs | QDir::NoDotAndDotDot | QDir::AllEntries |
                       QDir::Hidden);
    d_model->setSorting(QDir::Name | QDir::DirsFirst | QDir::IgnoreCase |
                        QDir::LocaleAware);
  ...
  if ((!sourceFile.exists()) && (!sourceRemote)) {
    sourceVisible = false;
    uiG.label_sourceError->setText(
        "<font color=red>" + tr("This directory does not exist") + "</font>");
  }
  ...
  if ((!destFile.exists()) && !(destRemote)) {
    ...
    uiG.label_destError->setText(
        "<font color=red>" +
        tr("This directory does not exist", "Information message") + "</font>");
    if (CurrentSnapshotString == "not available")
      deleteVisible = false; // disable the delete button
    restoreVisible = false;  // disable the restore button
  }
  // If source does not have read/enter permissions
  if (((!sourceFile.isReadable()) || (!sourceFile.isExecutable())) &&
      (!sourceRemote) && (sourceFile.exists())) {
    sourceVisible = false;
    uiG.label_sourceError->setText(
        "<font color=red>" +
        tr("I do not have sufficient permissions to read this directory",
           "Information message") +
        "</font>");
    restoreVisible = false; // disable the restore button
  }
  // If destination does not have read/enter permissions
  if (((!destFile.isReadable()) || (!destFile.isExecutable())) &&
      (!destRemote) && (destFile.exists())) {
    destVisible = false;
    uiG.label_destError->setText(
        "<font color=red>" +
        tr("I do not have sufficient permissions to read this directory",
  ...
  if (Operation[currentOperation]->GetTypeSync()) {
    syncTYPE = true;
    uiG.label_destination->setText(
        tr("sync dir A", "full phrase: sync dir a: <DIRECTORY_A>") + ": ");
    uiG.label_source->setText(
        tr("sync dir B", "full phrase: sync dir a: <DIRECTORY_b>") + ": ");
    uiG.pushButton_viewLog->setToolTip(
        tr("View the logfile of the selected sync task",
           "'view log' pushbutton tooltip"));
    deleteVisible = false;
    restoreVisible = false;
...
void manageDialog::SnapshotsListSelected() {
  ...
  if (CurrentSnapshotNo < 0) {
    ...
    CurrentSnapshotString =
        Operation[currentOperation]->GetSnapshotsListItem(CurrentSnapshotNo);

    if (WINrunning && Operation[currentOperation]->GetRemote())
      CurrentSnapshotDirectory = dest +
                                 snapDefaultDir.replace(SLASH, XnixSLASH) +
                                 CurrentSnapshotString + XnixSLASH + sourceLast;
    else
      CurrentSnapshotDirectory =
          dest + snapDefaultDir + CurrentSnapshotString + SLASH + sourceLast;

    if (CurrentSnapshotString ==
        "") // if there is no execution time of the current snapshot
...
void manageDialog::calculateDifferences() {
  ...
  if (calcdiffthread->isRunning()) {
    uiG.textBrowser_snapshot->setText(
        tr("Please wait until the thread is terminated"));
  ...
  if (!snapCalculated.at(
          CurrentSnapshotNo)) // if the differences have not been calculated yet
  {
    ...
    calcdiffthread->passParams(dest, dest, CurrentSnapshotDirectory,
                               CurrentSnapshotString, sourceLast, source,
                               CurrentSnapshotNo, currentSnaps, syncTYPE);
    calcdiffthread->start();
  } else
    uiG.textBrowser_snapshot->setText(
        calculatedDifferences.at(CurrentSnapshotNo));
}
...
void manageDialog::calcDiffEnd() {
  ...
  calculatedDifferences.replace(CurrentSnapshotNo,
                                calcdiffthread->returnResult());
  uiG.textBrowser_snapshot->setText(
      calculatedDifferences.at(CurrentSnapshotNo));

  if (abortCalcThread)
    uiG.textBrowser_snapshot->append("<p align=\"center\"><font color=red><b>" +
  ...
  uiG.pushButton_calculate_diffs->setToolTip(
      tr("Calculate the differences between the source and the selected "
         "snapshot"));
}
...
void manageDialog::refreshDifWindow(const QString &newText,
                                    const bool &appendText) {
  if (appendText)
...
void manageDialog::updateSnapshots() {
  ...
  calculatedDifferences
      .clear(); // this holds the calculated differences text for each snapshot
  ...
  bool emptySnapList = Operation[currentOperation]
                           ->SnapshotsListIsEmpty(); // Is the list empty ??

  // first check if there is a last execution time available but no snapshots
  // are declared and add a new snapshot with the last execution date-time
  time = (Operation[currentOperation]->GetLastExecutionTime())
  ...
  if (!(time == "") && (emptySnapList)) {
    ...
    currentSnaps =
        Operation[currentOperation]
            ->GetSnapshotsListSize(); // this is the current number of snapshots
    emptySnapList = Operation[currentOperation]
                        ->SnapshotsListIsEmpty(); // Is the list empty ??
  }
  ...
  while (count < currentSnaps) {
    QDateTime currentSnapTime = QDateTime::fromString(
        Operation[currentOperation]->GetSnapshotsListItem(count),
        "yyyyMMddhhmmss");
    if ((Operation[currentOperation]->GetSnapshotsListItem(count)) ==
        "") // if there is no snapshot time for a reason
      uiG.listWidget_dates->addItem(
          tr("not available", "refers to a date-time"));
    else
      uiG.listWidget_dates->addItem(
          currentSnapTime.toString("yyyy/MM/dd - hh:mm:ss"));
